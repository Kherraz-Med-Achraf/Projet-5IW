...
export class YousignService {
  ...
  /* helper HTTP -------------------------------------------------------- */
  private async call<T>(endpoint: string, init: RequestInit & { json?: unknown } = {}): Promise<T> {
  ...
  /* 3️⃣  Ajout d’un signer (simple + no_code + redirect_urls) ----------- */
  private addSigner(
    reqId: string,
    ...
    return this.call<SignerRes>(
      `signature_requests/${reqId}/signers`,
      {
        ...
        json  : {
          ...
          authentication_mode: 'no_code',          // ✅ requis pour “simple”
          redirect_urls: {                          // ✅ objet demandé par l’API
  ...
  verifyWebhookSignature(raw: string, sig: string) {
    ...
    const calc = crypto.createHmac('sha256', this.HOOK).update(raw, 'utf8').digest('hex');
