...
describe('PresenceController', () => {
  ...
  describe('validateSheet', () => {
    ...
    it('should handle empty present children list', async () => {
      const sheetId = 1;
      const dto = { presentChildIds: [] };
  ...
  describe('findByDate', () => {
    ...
    it('should return null for non-existent date', async () => {
      ...
      mockPresenceService.findByDate.mockResolvedValue(null);

      const result = await controller.findByDate(date);

      expect(service.findByDate).toHaveBeenCalledWith('2025-12-31');
    ...
    it('should handle invalid date format', async () => {
      ...
      // Should be handled by validation pipe in real scenario
      const result = await controller.findByDate(date);
      
      expect(service.findByDate).toHaveBeenCalledWith('invalid-date');
    });
  });

  describe('phone number masking', () => {
    it('should mask parent phone numbers in response', async () => {
      const dto = { date: '2025-01-15' };
      const sheetWithPhone = {
        ...mockSheet,
        records: [{
          ...mockSheet.records[0],
          child: {
            ...mockSheet.records[0].child,
            parent: { phone: '+33123456789' },
          },
        }],
      };

      mockPresenceService.createSheet.mockResolvedValue(sheetWithPhone);

      const result = await controller.createSheet(dto, mockRequest as any);

      // The controller should mask phone numbers
      expect(result.records[0].child.parent.phone).toBe('+33123***789');
    });

    it('should handle missing parent phone', async () => {
      const dto = { date: '2025-01-15' };
      const sheetNoPhone = {
        ...mockSheet,
        records: [{
      ...
      mockPresenceService.createSheet.mockResolvedValue(sheetNoPhone);

      const result = await controller.createSheet(dto, mockRequest as any);

      expect(result.records[0].child.parent.phone).toBeNull();
    });
  });
