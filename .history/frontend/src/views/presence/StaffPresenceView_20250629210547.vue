...
function initGrid() {
  ...
  const columns = [
    ...
    },
    {
      name: 'childId',
      hidden: true
      // data[2] = childId - colonne cachée mais accessible
    },
    {
      name: 'Présent',
      width: '15%',
      formatter: (cell: any, row: any) => {
        ...
        // Pour accéder aux autres données : row.cells[index].data
        const isPresent = cell
        const childId = row.cells[2]?.data  // colonne childId (index 2)
        const justificationType = row.cells[4]?.data  // colonne justification type
