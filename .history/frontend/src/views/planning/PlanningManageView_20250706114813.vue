...
async function handleCancelCourse(course: any) {
  ...
  if (course.cancelled) {
    try {
      ...
      success('Cours réactivé avec succès !', 'Les enfants transférés ont été automatiquement restaurés.')
    } catch (err: any) {
      console.error('=== ERREUR RÉACTIVATION ===', err)
      // Afficher l'erreur à l'utilisateur
      error('Erreur lors de la réactivation', err.message || 'Erreur inconnue')
    }
    return
  }
  ...
  if (courseData.children && courseData.children.length > 0) {
    try {
      ...
      if (alternatives.length > 0) {
        ...
        closeModal()
        info('Cours alternatifs trouvés', `${alternatives.length} cours alternatifs disponibles pour le transfert.`)
      } else {
        // Pas d'alternatives, confirmer l'annulation directe
        showConfirmModal.value = true
        closeModal()
        warning('Aucune alternative', 'Aucun cours alternatif trouvé. Voulez-vous annuler le cours sans transfert ?')
      }
    } catch (err: any) {
      console.error('Erreur lors de la recherche d\'alternatives:', err)
      error('Erreur de recherche', 'Impossible de trouver des cours alternatifs.')
      return
    }
  } else {
    ...
    try {
      ...
      await loadEvents()
      closeModal()
      success('Cours annulé', 'Le cours a été annulé avec succès.')
    } catch (err: any) {
      console.error('Erreur lors de l\'annulation:', err)
      error('Erreur lors de l\'annulation', err.message || 'Erreur inconnue')
    }
  }
}
...
async function handleIndividualTransfers(transfers: Record<number, string>) {
  ...
  if (!cancelledCourseId) {
    ...
    error('Erreur de transfert', 'ID du cours non trouvé')
    return
  }
  ...
  try {
    ...
    if (transferCount > 0) {
      ...
      success('Transferts terminés !', `${transferCount} enfant(s) transféré(s) et cours annulé automatiquement.`)
    } else {
      info('Aucun transfert', 'Aucun enfant n\'a été transféré.')
    }
