...
async function main() {
  ...
  console.log('✅ Director créé avec succès');
  ...
  for (let i = 0; i < smMails.length; i++) {
    ...
    console.log('✅ Service-manager créé avec succès');
  }
  ...
  console.log('✅ Secretary créé avec succès');
  ...
  /* ---------------------------------------------------------------------- */
  /* 6.1. PARENT EXISTANT AVEC MOT DE PASSE EXPIRÉ (PARENT20)              */
  /* ---------------------------------------------------------------------- */
  
  // Modifier parent20@example.com pour avoir un mot de passe expiré
  ...
  await prisma.user.update({
    ...
    data: {
      passwordChangedAt: expiredDate, // Mot de passe expiré depuis 65 jours
    },
  });

  console.log('✅ Parent20 modifié avec mot de passe EXPIRÉ → parent20@example.com / Test1234!!!! (expiré depuis 65 jours)');
