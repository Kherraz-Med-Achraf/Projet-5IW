...
async function ensureCSRFToken(): Promise<string | null> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // CRITICAL: Synchroniser manuellement le cookie côté JavaScript
      // pour Chrome qui ne synchronise pas bien les cookies automatiquement
      if (csrfToken) {
        document.cookie = `csrf_token=${csrfToken}; path=/; SameSite=Strict; max-age=${2 * 24 * 60 * 60}`;
        console.log('[API] Cookie CSRF synchronisé manuellement pour Chrome');
      }
    } else {
