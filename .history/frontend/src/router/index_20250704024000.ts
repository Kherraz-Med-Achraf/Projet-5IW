...
router.beforeEach(async (to, from, next) => {
  ...
  // 4) Si on navigue vers une route authentifiée, s'assurer que l'utilisateur est chargé
  if (requiresAuth && auth.isAuthenticated) {
    // Si l'utilisateur n'est pas encore chargé, attendre un peu
    if (!auth.user) {
      console.log('Router: Auth token exists but user not loaded, waiting...');
      // Attendre un petit délai pour que l'auth store se stabilise
      await new Promise(resolve => setTimeout(resolve, 150));
      
      // Si toujours pas d'utilisateur après le délai, rediriger vers login
      if (!auth.user) {
        console.warn('Router: User still not loaded after delay, redirecting to login');
        return next({ name: "Login" });
      }
    }
  }

  // 5) SÉCURITÉ CRITIQUE: Vérifier les rôles interdits (ex: empêcher les enfants d'accéder au journal)
  ...
  // 6) Si un rôle précis est requis
