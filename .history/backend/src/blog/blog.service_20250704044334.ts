...
export class BlogService {
  ...
  async createPost(dto: CreateBlogPostDto, authorId: string): Promise<BlogPostResponseDto> {
    ...
    const sanitizedTitle = DOMPurify.sanitize(dto.title.trim(), { ALLOWED_TAGS: [] });
    const sanitizedDescription = DOMPurify.sanitize(dto.description.trim(), { ALLOWED_TAGS: [] });
  ...
  async updatePost(postId: string, dto: CreateBlogPostDto, userRole: Role, userId: string): Promise<BlogPostResponseDto> {
    ...
    const updateData: any = {
      title: DOMPurify.sanitize(dto.title.trim(), { ALLOWED_TAGS: [] }),
      description: DOMPurify.sanitize(dto.description.trim(), { ALLOWED_TAGS: [] }),
    };
