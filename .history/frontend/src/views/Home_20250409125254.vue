<template>
  <div>
    ...
    <!-- Affichage du QR code si OTP activé via l'action (après avoir cliqué) -->
...
import * as QRCode from 'qrcode'  // Utilisé pour générer le QR code côté navigateur

const auth = useAuthStore()
const toast = useToast()
const router = useRouter()

// On initialise otpEnabled à false (la case reste décochée)
...
// Fonction pour générer le QR code à partir du secret OTP fourni par le backend
...
async function toggleOtp() {
  // Si l'utilisateur coche la case -> on active l'OTP
  if (otpEnabled.value) {
    loading.value = true
    try {
      const token = localStorage.getItem('token')
      // Appel à la route POST /auth/enable-otp
      ...
      // On met à jour le QR code et le secret à partir de la réponse
      qrCodeDataUrl.value = data.qrCodeDataUrl || ''
      secret.value = data.secret || ''
      // On met à jour auth.user.otpSecret pour refléter l'activation OTP
      if (auth.user) {
    ...
    // La checkbox vient d'être décochée -> désactiver l'OTP
    loading.value = true
    try {
