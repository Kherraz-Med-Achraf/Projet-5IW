  ...
  <div class="profile-wrapper">
    ...
    <div class="profile-content" id="main-content">
      <div class="content-grid">
        ...
        <div v-if="auth.user?.role === 'STAFF'" class="profile-section alert-section" id="alert-section" aria-labelledby="alert-title">
          <div class="section-header">
            <h2 id="alert-title">
...
async function checkJournalAlerts() {
  ...
  try {
    ...
    // 6) Afficher l'alerte si nécessaire
    if (missing.length) {
      const names = missing
        .map((c) => `${c.firstName} ${c.lastName}`)
        .join(", ");
      toast.warning(`Journaux manquants pour : ${names}`, {
        timeout: 10000 // Affichage plus long pour les alertes importantes
      });
    }
  } catch (error) {
