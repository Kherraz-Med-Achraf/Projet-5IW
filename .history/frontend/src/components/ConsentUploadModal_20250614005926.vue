...
<template>
  ...
  /* Charge la liste complète des parents (appelé à l'ouverture) */
  async function loadParents() {
    if (parents.value.length) return               // déjà chargé
    try {
      const res = await fetch('/parents', {
        headers,
        credentials: 'include',
      })
      if (res.ok) parents.value = await res.json()
      else console.error('Erreur /parents :', res.status, await res.text())
    } catch (err) {
      console.error('Network error /parents', err)
    }
  }
  ...
  function open()  {
    loadParents().then(() => dialog.value?.showModal())
  }
  function close() {
    dialog.value?.close()
    name.value = ''
