...
function initGrid() {
  ...
  const isValidated = sheet.value.validatedAtStaff
    ...
    // Prepare data with full record objects for easier access
  const data = sheet.value.records.map(rec => ({
    lastName: rec.child.lastName,
    firstName: rec.child.firstName,
    childId: rec.childId,
    present: rec.present,
    justificationType: rec.justification?.type || null,
    justificationMotif: rec.justification?.motif || null,
    record: rec // Keep full record for reference
  }))
  ...
  const columns = [
    { 
      name: 'Nom', 
      width: '25%',
      data: (row: any) => row.lastName
    },
    { 
      name: 'Prénom', 
      width: '25%',
      data: (row: any) => row.firstName
    },
    {
      name: 'Présent',
      width: '15%',
      formatter: (cell: any, row: any) => {
        ...
        const justificationType = row.justificationType
        ...
        if (isValidated) {
          ...
          const checked = store.presentChildIds.includes(childIdNum) ? 'checked' : ''
          return html(`
            <div class="checkbox-container">
              <input 
                ...
                value="${childId}" 
                ${checked}
                onchange="handlePresenceChange('${childId}', this.checked)"
                class="presence-checkbox"
              />
              <label class="checkbox-label">${store.presentChildIds.includes(childIdNum) ? 'Présent' : 'Absent'}</label>
            </div>
          `)
        }
      }
  ...
  if (isValidated) {
    columns.push({
      ...
      width: '35%',
      formatter: (cell: any, row: any) => {
        const motif = row.justificationMotif
        const type = row.justificationType
...
function setupGlobalCheckboxHandler() {
  ;(window as any).handlePresenceChange = (childId: string, checked: boolean) => {
