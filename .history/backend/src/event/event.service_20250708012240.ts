...
export class EventService {
  ...
  async confirmStripePayment(registrationId: string, sessionId: string) {
    const reg = await this.prisma.eventRegistration.findUnique({
      where: { id: registrationId },
      include: { parentProfile: true, event: true },
    });
    ...
    if (reg.paymentStatus === PaymentStatus.PAID) {
      console.log(`Payment already confirmed for registration ${registrationId}`);
      return;
    }

    // Vérifier que la session Stripe correspond
    if (reg.stripeSessionId && reg.stripeSessionId !== sessionId) {
    ...
    await this.prisma.eventRegistration.update({
      ...
      data: { 
        ...
        stripeSessionId: sessionId, // S'assurer que l'ID est bien enregistré
      },
    });

    // Envoyer l'email de confirmation
    await this._sendRegistrationMail(registrationId);

    console.log(`✅ Payment confirmed via webhook for registration ${registrationId}`);
  }

  /** Gestion des sessions Stripe expirées */
  async handleExpiredStripeSession(registrationId: string) {
    const reg = await this.prisma.eventRegistration.findUnique({
      where: { id: registrationId },
    ...
    if (!reg) {
      console.warn(`Registration ${registrationId} not found for cleanup`);
      return;
    }

    if (reg.paymentMethod !== PaymentMethod.STRIPE || reg.paymentStatus !== PaymentStatus.PENDING) {
      console.warn(`Registration ${registrationId} is not a pending Stripe payment`);
      return;
    }
    ...
    try {
      ...
      console.log(`🗑️ Expired registration ${registrationId} cleaned up successfully`);
    } catch (error) {
  ...
  async handleFailedStripePayment(registrationId: string, paymentIntentId: string, errorMessage: string) {
    const reg = await this.prisma.eventRegistration.findUnique({
      where: { id: registrationId },
      include: { event: true, parentProfile: { include: { user: true } } },
    });
    ...
    if (!reg) {
      console.warn(`Registration ${registrationId} not found for failed payment`);
      return;
    }

    if (reg.paymentMethod !== PaymentMethod.STRIPE) {
      console.warn(`Registration ${registrationId} is not a Stripe payment`);
      return;
    }
    ...
    try {
      ...
      await this.mail.sendMail(
        ...
        `<p>Bonjour,</p>
         ...
         <p><strong>Raison :</strong> ${errorMessage}</p>
      ...
      console.log(`❌ Failed payment handled for registration ${registrationId}: ${errorMessage}`);
    } catch (error) {
      console.error(`Failed to handle failed payment for registration ${registrationId}:`, error);
      throw error;
    }
  }
  ...
  async handleStripeChargeback(registrationId: string, chargeId: string, paymentIntentId: string) {
    const reg = await this.prisma.eventRegistration.findUnique({
      where: { id: registrationId },
      include: { event: true, parentProfile: { include: { user: true } } },
    });
    ...
    try {
      ...
      console.log(`⚖️  Chargeback handled for registration ${registrationId}: ${chargeId}`);
    } catch (error) {
