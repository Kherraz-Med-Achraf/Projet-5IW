...
export class AuthService {
  ...
  async initiateLogin(email: string, password: string) {
    ...
    // Vérifier l'expiration APRÈS validation des identifiants
  ...
  async registerWithRole(dto: RegisterByInviteDto, forcedRole: Role) {
    ...
    // Validation supplémentaire des données parent
    if (!parentData.firstName || !parentData.lastName || !parentData.phone) {
      throw new BadRequestException('Les informations du parent sont incomplètes.');
    }
  ...
  async changePassword(userId: string, currentPwd: string, newPwd: string) {
    ...
    console.log(`[CHANGE_PASSWORD] Utilisateur change son mot de passe`);
    console.log(`[CHANGE_PASSWORD] Ancienne date passwordChangedAt: ${user.passwordChangedAt?.toISOString()}`);
    ...
    console.log(`[CHANGE_PASSWORD] Nouvelle date passwordChangedAt: ${newPasswordChangedAt.toISOString()}`);
    console.log(`[CHANGE_PASSWORD] Mot de passe mis à jour avec succès`);
  ...
  async updateProfile(
    ...
    switch (user.role) {
      ...
      case 'CHILD':
        if (user.childProfile) {
          // Pour les enfants, on exclut le téléphone
          const childData: { firstName?: string; lastName?: string } = {};
          if (sanitizedData.firstName) childData.firstName = sanitizedData.firstName;
          if (sanitizedData.lastName) childData.lastName = sanitizedData.lastName;
  ...
  private checkPasswordExpiration(user: any): boolean {
    // Ne pas vérifier l'expiration pour les comptes CHILD
    if (user.role === 'CHILD') {
      return false;
    }
    ...
    console.log(`[PASSWORD_EXPIRATION] Vérification expiration mot de passe:`);
