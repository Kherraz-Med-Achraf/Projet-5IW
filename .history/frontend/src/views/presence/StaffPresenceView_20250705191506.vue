...
const presentCount  = computed(() => sheet.value?.records.filter(r => r.present || r.justification?.type === 'LATENESS').length || 0)
const latenessCount = computed(() => sheet.value?.records.filter(r => r.justification?.type === 'LATENESS').length || 0)
const absenceCount  = computed(() => sheet.value?.records.filter(r => !r.present && r.justification?.type !== 'LATENESS').length || 0)
...
async function confirmValidation() {
  ...
  try {
    ...
    // Fermer la modale après validation réussie
    showModal.value = false
    
    // Afficher une notification de succès
    notify.showNotification('Feuille validée avec succès', 'success')
    
    // Optionnel: rediriger vers la page de sélection des feuilles
    // router.push('/presence')
  } catch (error: any) {
    notify.showNotification(error.message || 'Erreur lors de la validation', 'error')
  } finally {
