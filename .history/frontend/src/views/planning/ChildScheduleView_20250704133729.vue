  ...
  <main class="profile-container">
    ...
    <div class="profile-content" id="main-content">
      <div class="content-grid">
        ...
        <!-- Calendrier -->
        <section v-if="events.length" class="profile-section calendar-section" id="calendar-view" aria-labelledby="calendar-title">
          <div class="section-header">
...
const children = computed(() => auth.user?.children || [])
...
const fcOptions: any = {
  ...
  eventContent(info: any) {
    ...
    if (info.view.type === 'dayGridMonth') {
      let label = 'Activité'
      let bgColor = '#4338ca' // bleu pour les activités
      let textColor = 'white'
      
      if (isVacation) {
        label = info.event.title.toLowerCase().includes('férié') ? 'Jour férié' : 'Vacances scolaires'
        bgColor = '#FFB74D' // orange pour vacances/fériés
        textColor = 'black'
      } else if (isCancelled) {
