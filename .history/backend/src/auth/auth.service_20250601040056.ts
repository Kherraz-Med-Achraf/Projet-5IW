...
import { ChildService } from '../child/child.service';          // ← NEW
...
export class AuthService {
  constructor(
    ...
    private childService: ChildService,                        // ← NEW
  ) {}
  ...
  async registerParent(dto: RegisterParentDto) {
    const {
      ...
      children, emergencyContacts, ...parentData
    } = dto;
    ...
    /* 1. Création du User + ParentProfile (SANS enfants) */
    ...
    /* 2. Création de chaque enfant + compte utilisateur associé */
    for (const c of children) {
      await this.childService.createForParent(parentUser.parentProfile.id, c);
    }

    return { message: 'Compte parent (et comptes enfants) créés et actifs.' };
  }
  ...
  async registerWithRole(dto: RegisterByInviteDto, forcedRole: Role) {
    ...
    const contactsCreates = (Array.isArray(emergencyContacts) ? emergencyContacts : [])
    ...
    if (forcedRole === Role.PARENT) {
      /* 1. User + ParentProfile (sans enfants) */
      const parentUser = await this.prisma.user.create({
        data: {
          email,
          password: passwordHash,
          ...
          parentProfile: {
            create: {
              ...parentData,
              ...(contactsCreates.length && {
                emergencyContacts: {
                  create: contactsCreates.map(c => ({
      ...
      for (const c of childCreates) {
        await this.childService.createForParent(parentUser.parentProfile.id, {
