...
<template>
  <div class="max-w-3xl mx-auto p-6 space-y-8">
    ...
    <!-- Formulaire complet (toujours les 3 étapes) -->
    <div v-else>
...
const form = reactive<any>({
  ...
  email:'', password:'', passwordConfirm:''Z,
})
/* ----------------------------- */
...
onMounted(async ()=>{
  ...
  if(token){
    ...
    try{
      ...
      form.email = data.email  // email pré-rempli et bloqué
    }catch(e:any){
...
async function submit(){
  ...
  /* ----- valeurs communes (adresse, contacts, …) ----- */
  const fullAddress =
  ...
  if(inviteToken.value){
    // mêmes données que registerParent + le token
    const payload = {
