...
export class ChildService {
  ...
  async findAllForParent(parentProfileId: number): Promise<Child[]> {
    const children = await this.prisma.child.findMany({ where: { parentProfileId } });
    console.log('🔍 [BACKEND] findAllForParent - parentProfileId:', parentProfileId);
    console.log('🔍 [BACKEND] findAllForParent - enfants trouvés:', children);
    return children;
  }
  ...
  /**
   * Met à jour uniquement le droit à l'image d'un enfant pour un parent
   */
  async updateImageConsentForParent(
    parentProfileId: number,
    childId: number,
  ...
  async remove(id: number): Promise<void> {
    // D'abord récupérer l'enfant pour avoir l'userId s'il existe
  ...
  /**
   * Renvoie tous les enfants pour lesquels l'utilisateur (userId) est référent.
   */
  async findAllReferent(userId: string): Promise<Child[]> {
    return this.prisma.child.findMany({
      where: {
        referents: {
          some: {
