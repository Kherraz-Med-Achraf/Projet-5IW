...
export class InvitationService {
  ...
  /**
   * Génère une invitation, la stocke en base, et envoie l’e-mail d’invitation.
   * @param email         Adresse e-mail de l’invité
   * @param roleToAssign  Rôle à attribuer lors de l’inscription (ex. Role.PARENT)
   * @param inviterId     ID de l’utilisateur qui génère l’invitation (SERVICE_MANAGER, DIRECTOR ou ADMIN)
   * @param expiresAt     Date d’expiration du lien d’invitation
   */
  async createInvitation(
    ...
    // Vérifier que l’e-mail n’est pas déjà utilisé par un User existant
    ...
    // Vérifier qu’il n’existe pas déjà d’invitation valide pour cette adresse
    ...
    // Créer la ligne Invitation en base
    ...
    // Construire le lien vers le front (configurable via FRONTEND_URL)
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
    const registrationLink = `${frontendUrl}/register?token=${token}`;

    // Préparer et envoyer l’e-mail d’invitation
