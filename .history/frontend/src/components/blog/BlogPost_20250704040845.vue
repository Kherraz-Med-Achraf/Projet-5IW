<template>
  <article class="blog-post">
    ...
    <div class="post-content">
      ...
      <div v-if="post.mediaUrl" class="post-media">
        <img
          ...
          tabindex="0"
          role="button"
          :aria-label="`Voir l'image en grand: ${post.title}`"
        />
        <video
          ...
          controls
          class="post-video"
          :aria-label="`Vidéo de l'article: ${post.title}`"
        ></video>
      </div>
    </div>

    <!-- Réactions -->
    <footer class="post-reactions" role="group" aria-label="Réactions à l'article">
      <div class="reactions-list">
        <button
          ...
          :aria-label="`${getReactionLabel(reactionType)} (${post.reactions[reactionType]} réactions)`"
          :aria-pressed="post.userReaction === reactionType"
          :title="`${getReactionLabel(reactionType)} - ${post.reactions[reactionType]} réactions`"
        >
    ...
    <div 
      ...
      role="dialog"
      aria-modal="true"
      :aria-label="`Image agrandie: ${post.title}`"
      tabindex="-1"
    >
      <div class="modal-content" @click.stop>
        <img 
          :src="fullMediaUrl" 
          :alt="`Image agrandie de l'article: ${post.title}`" 
          class="modal-image" 
        />
        <button 
          type="button" 
