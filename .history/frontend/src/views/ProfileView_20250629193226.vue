        ...
        <form @submit.prevent="saveContact" class="contact-form">
          ...
          <div class="form-group">
            ...
            <select
              ...
              required
            >
              <option disabled value="">Sélectionner la relation...</option>
              <option>Mère</option>
              <option>Père</option>
              <option>Sœur</option>
              <option>Frère</option>
...
function openContactModal(contact: any) {
  ...
  // Liste des relations prédéfinies
  const predefinedRelations = [
    'Mère', 'Père', 'Sœur', 'Frère', 'Grand-parent', 
    'Oncle / Tante', 'Cousin·e', 'Ami·e de la famille', 'Voisin·e'
  ];
  
  let relation = contact.relation || '';
  let relationOther = '';
  
  // Vérifier si la relation est prédéfinie
  if (predefinedRelations.includes(relation)) {
    // Relation standard - on la garde telle quelle
    relation = contact.relation;
  } else if (relation === 'Tante' || relation === 'Oncle') {
    // Mapper Tante/Oncle vers "Oncle / Tante"
    relation = 'Oncle / Tante';
  } else if (relation.startsWith('Autre:')) {
    // Relation personnalisée avec préfixe "Autre:" (format de l'inscription)
    relation = 'Autre';
    relationOther = contact.relation.replace('Autre: ', '').trim();
  } else if (relation && !predefinedRelations.includes(relation)) {
    // Relation personnalisée sans préfixe - traiter comme "Autre"
    relation = 'Autre';
    relationOther = contact.relation;
  } else if (!relation) {
