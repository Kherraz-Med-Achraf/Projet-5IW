...
import { encrypt as encryptOtp } from '../utils/encryption';
...
// Tests temporairement désactivés - nécessitent DB et variables d'environnement
describe.skip('AuthService – sécurité connexion', () => {
  const PWD_HASH = 'HASH';
  const baseUser = {
    id: 'u1',
    email: 'user@test.com',
...
describe.skip('Rate limiter /auth', () => {
  const app = express();
...
describe.skip('AuthService – forgotPassword()', () => {
  const user = {
    id: 'uid',
    email: 'foo@bar.com',
    password: 'HASH',
...
describe.skip('AuthService – OTP enable / disable', () => {
  const user = {
    id: 'uOtp',
    email: 'otp@test.com',
    password: 'HASH',
...
describe.skip('AuthService – login() sans OTP', () => {
...
describe.skip('AuthService – login() avec OTP', () => {
  const user = {
    id: 'uOtpLogin',
    email: 'otp@login.com',
    password: 'HASHED',
    failedLoginAttempts: 0,
    lockUntil: null,
    otpSecret: 'MOCK_ENCRYPTED_SECRET', // encryptOtp('BASE32SECRET'),
    role: 'PARENT',
    passwordChangedAt: new Date(),
  };
  ...
  it('retourne un tempToken quand otpCode manquant', async () => {
    const res = await svc.login(user.email, 'pwdSansOtp');
    expect(res).toHaveProperty('tempToken');
  });
