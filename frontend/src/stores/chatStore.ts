...
export const useChatStore = defineStore("chat", () => {
  ...
  async function createChatWith(userId: string) {
    ...
    try {
      ...
      // Attendre que l'événement newChat soit traité via WebSocket
      // Si pas reçu dans les 2 secondes, forcer le rafraîchissement
      ...
      const fallbackTimeout = setTimeout(async () => {
        if (!chatReceived) {
          clearInterval(checkInterval);
          console.warn("Événement newChat non reçu, rafraîchissement forcé");
          
          // Rafraîchir la liste des chats comme fallback
          await fetchChats();
  ...
  async function init() {
    ...
    try {
      await secureJsonCall(`${API}/auth/profile`);
              ...
              console.error("[ChatStore] Token invalide, nettoyage et arrêt");
      
      // Si le token est expiré, nettoyer et rediriger
    ...
    socket.on("newMessage", (msg: any) => {
      ...
      const already = messages[msg.chatId].some(
        (m) => m.id === (msg.msgId ?? "") || (m.id.startsWith("tmp_") && m.authorId === msg.authorId && m.content === msg.content)
      );
      if (already) {
        // Remplacer le message temporaire par le vrai si c'est le cas
        const tempIdx = messages[msg.chatId].findIndex(
          (m) => m.id.startsWith("tmp_") && m.authorId === msg.authorId && m.content === msg.content
        );
        if (tempIdx !== -1) {
