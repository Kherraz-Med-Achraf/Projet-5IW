...
import {
  ...
  export class ChatService {
    ...
    private async getStaffContacts(staffId: string): Promise<Contact[]> {
      ...
      // Parents dont l’enfant référence ce staff
      const children = await this.prisma.child.findMany({
        where: { referents: { some: { id: staffId } } },
        include: { parent: { select: { user: { select: { id: true, email: true, role: true } } } } },
      });
    ...
    /* ---------- matrice basique rôle → rôle -------------------------------- */
  
    private isAllowed(senderRole: string, receiverRole: string, senderId: string, receiverId: string) {
