  ...
  actions: {
    ...
    async register(data: { email: string; password: string; username?: string }) {
      ...
      try {
        ...
        if (!response.ok) {
          // On lève l'erreur pour que le composant Vue puisse la catch
          throw new Error(resData.message || "Erreur lors de l'inscription")
        }
        // on ne notifie plus ici, c'est le composant qui s'en charge
        return resData
      } catch (error: any) {
        this.error = error.message
        // on ne notifie plus ici non plus
    ...
    async enableOtp() {
      ...
      try {
        ...
        // Pas de refresh ici pour tránh invalidation token
        return data
      } catch (error: any) {
    ...
    async disableOtp() {
      ...
      try {
        ...
        const data = await response.json()   // <-- on garde l’objet complet
        if (!response.ok) {
