...
export class YousignService {
  ...
  /* ----------------------------------------------------------- *
   * 1. Upload PDF vers Yousign   ⬅️ corrigé (Buffer, pas Blob)
   * ----------------------------------------------------------- */
  private async uploadFile(filePath: string): Promise<string> {
    const form   = new FormData();
    const buffer = await fs.readFile(filePath);          // Buffer OK

    // Yousign déduit le content-type et garde le nom d’origine
    ...
    const { id } = (await res.json()) as UploadedFileRes;
    return id;
  }
