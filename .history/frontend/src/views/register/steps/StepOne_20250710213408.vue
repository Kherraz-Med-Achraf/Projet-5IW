<template>
  <div class="register-step">
    <div class="register-step__container">
      <h1 class="register-step__title">Informations du parent</h1>
      <h2 class="register-step__subtitle">Informations personnelles</h2>
      ...
      <h2 class="register-step__subtitle">Adresse</h2>
      <!-- Adresse -->
      <div class="register-step__grid">
        <BaseInput
      ...
      <h2 class="register-step__subtitle">Responsabilité légale</h2>
      <!-- Responsabilité légale -->
      <div class="register-step__section">
        <div class="register-step__select-wrapper">
          <label for="legal-responsibility-select" class="register-step__label">Responsabilité légale</label>
          <select
            id="legal-responsibility-select"
            v-model="form.legalResponsibility"
            :class="{ error: errors.legalResponsibility }"
            aria-describedby="legal-responsibility-error"
          >
          ...
          <div 
            v-if="errors.legalResponsibility" 
            id="legal-responsibility-error"
            class="error-message"
            role="alert"
            aria-live="polite"
          >
      ...
      <!-- Contacts d'urgence -->
      <div class="register-step__section">
        <h2 class="register-step__subtitle">Contacts d'urgence</h2>
        <div
          v-for="(contact, index) in form.emergencyContacts"
          :key="`ec-${index}`"
          class="register-step__emergency-item"
        >
          <h3 class="register-step__contact-title">
          ...
          <div class="register-step__grid">
            ...
            <div class="register-step__relation">
              <label :for="`contact-relation-${index}`" class="register-step__label">Lien</label>
              <div class="register-step__select-wrapper">
                <select
                  ...
                  :aria-describedby="`contact-relation-error-${index}`"
                >
                ...
                <div
                  v-if="
                    ...
                    errors.emergencyContacts[index]?.relation
                  "
                  :id="`contact-relation-error-${index}`"
                  class="error-message"
                  role="alert"
                  aria-live="polite"
                >
          ...
          <button
            ...
            :aria-label="`Supprimer le contact d'urgence ${index + 1}`"
            type="button"
          >
            ✕
          </button>
...
function validateForm() {
  ...
  if (!f.phone) {
    ...
    if (!phoneRegex.test(f.phone.replace(/\s/g, ''))) {
      errors.phone = "Format de téléphone invalide (ex: 0123456789 ou +33123456789)";
      valid = false;
    }
  }
  if (!f.address.number) {
  ...
  if (!f.address.postal) {
    ...
    if (!postalRegex.test(f.address.postal)) {
      errors.addressPostal = "Code postal invalide (5 chiffres requis)";
      valid = false;
    }
  }
  if (!f.address.city) {
...
function goToStep2() {
  ...
  // SYNCHRONISATION FORCÉE AVANT NAVIGATION
  console.log('🔄 Synchronisation forcée avant navigation vers step 2');
  if (registerStore.form) {
    Object.assign(registerStore.form, form);
    console.log('✅ Données synchronisées:', registerStore.form);
    console.log('✅ Étape 1 complète:', registerStore.isStepOneComplete());
    console.log('✅ Peut accéder étape 2:', registerStore.canAccessStep(2));
  }
  
  // Vérifier une dernière fois avant navigation
  if (!registerStore.canAccessStep(2)) {
    console.error('❌ Impossible d\'accéder à l\'étape 2, données manquantes');
...
function handleAddEmergencyContact() {
  // Laisser le store gérer la validation et l'ajout
