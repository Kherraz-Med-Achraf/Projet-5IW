<template>
  <main class="profile-container" role="main">
    ...
    <TransferChildrenModal 
      ...
      @transfer="handleTransferChildren"
    />
  </main>
</template>
...
async function handleCancelCourse(course: any) {
  // Utiliser le nouveau workflow d'annulation avec transfert automatique
  showModal.value = false
  
  const result = await planning.cancelCourseWithTransfer(course.id)
  
  if (result.cancelled && result.hasChildren) {
    // Si le cours avait des enfants, proposer le transfert
    transferData.value = {
