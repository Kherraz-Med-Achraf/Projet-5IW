...
async function fetchChildren(): Promise<void> {
  try {
    const { secureJsonCall, API_BASE_URL } = await import('@/utils/api');
    console.log('[EventListView] Appel /children pour récupérer les enfants du parent...');
    children.value = await secureJsonCall(`${API_BASE_URL}/children`);
    console.log('[EventListView] Enfants récupérés:', children.value);
    console.log('[EventListView] Nombre d\'enfants:', children.value.length);
  } catch (error) {
