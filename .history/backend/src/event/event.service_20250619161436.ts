...
export class EventService {
  ...
  async register(eventId: string, userId: string, dto: RegisterEventDto, origin: string) {
    ...
    await this.prisma.eventRegistration.update({ where: { id: result.id }, data: { paymentStatus: PaymentStatus.PAID } });
    await this._sendRegistrationMail(result.id);

    const kids = await this.prisma.eventRegistrationChild.findMany({ where: { registrationId: result.id }, include: { child: true } });
    return {
      eventTitle: ev.title,
  ...
  async confirmStripe(sessionId: string) {
    ...
    if (reg.paymentStatus === PaymentStatus.PAID) return; // déjà traité
