...
export class PlanningService {
  ...
  async previewExcel(
    ...
    // Fonction pour décoder les entités HTML et nettoyer les messages
    ...
    for (const sheetName of sheetNames) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        for (let idx = 1; idx <= lastIdx; idx++) {
          ...
          // Amélioration du parsing des cellules Excel
          let actRaw = '';
          let namesRaw = '';
          
          // Vérifier différents formats possibles
          if (cell.includes('–')) {
            // Format standard: "Activité – Enfants"
            [actRaw, namesRaw] = cell.split('–').map((s) => s.trim());
          } else if (cell.includes('-')) {
            // Format alternatif: "Activité - Enfants"
            [actRaw, namesRaw] = cell.split('-').map((s) => s.trim());
          } else {
            // Pas de séparateur trouvé - vérifier si c'est juste une activité
            actRaw = cell.trim();
            namesRaw = '';
          }
          ...
          if (act.toLowerCase() === 'pause') {
            ...
            if (!namesRaw || namesRaw === '') {
              // Pas d'enfants spécifiés - peut-être une activité administrative
              if (act.toLowerCase().includes('pause') || 
                  act.toLowerCase().includes('réunion') || 
                  act.toLowerCase().includes('administratif') ||
                  act.toLowerCase().includes('préparation')) {
                childrenNames = [];
              } else {
                validationErrors.missingChildrenSlots.push({
              ...
              childrenNames = namesRaw
                .split(',')
                .map((s) => s.trim())
                .filter((n) => n !== '' && n.toLowerCase() !== 'accueil')
                .map((n) => normalizeName(n));
              
              // cas spécial "tous"
              if (childrenNames.some(cn => cn.includes('tous') || cn.includes('tout'))) {
              ...
              childrenNames.forEach((cn) => {
                ...
                if (normalizedChildName && !childKeys.has(normalizedChildName)) {
                  validationErrors.unrecognizedChildren.push({
                    name: cleanMessage(cn),
                    day: sheetName,
                    timeSlot: timeSlotNames[idx - 1],
                    activity: cleanMessage(act),
                    line: i + 1,
                    column: idx + 1,
                  });
                }
    ...
    if (validationErrors.missingStaff.length > 0 || 
      ...
      if (validationErrors.missingChildrenSlots.length > 0) {
        ...
        validationErrors.missingChildrenSlots.forEach(slot => {
          ...
          groupedMissing.get(cleanChild)!.push({
            day: slot.day,
