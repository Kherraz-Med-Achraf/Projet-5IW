...
export class PresenceController {
  ...
  async createSheet(
    ...
    const sheet = await this.service.createSheet(dto.date, req.user.id, req.user.role);
  ...
  async validateSheet(
    ...
    const sheet = await this.service.validateSheet(sheetId, dto.presentChildIds, req.user.id, req.user.role);
    return this._mapParentPhones(sheet);
  }
  ...
  async justify(
    ...
    const sheet = await this.service.justify(recordId, dto, file?.path, req.user.role);
    return this._mapParentPhones(sheet);
  }
  ...
  async downloadJustification(
    ...
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(pdf|jpg|jpeg|png|webp)$/i;
    ...
    const uploadsDir = resolve(process.cwd(), 'uploads', 'justifications');
    ...
    // Headers sécurisés
    const safeContentType = this.getSecureContentType(extname(safeFilename));
    res.set({
