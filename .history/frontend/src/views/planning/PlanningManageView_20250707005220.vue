...
const staffList = computed(() => {
  console.log('Raw staff data:', planning.staffList)
  
  // Éliminer les doublons par userId avec une logique plus robuste
  const uniqueStaff = planning.staffList.filter((staff, index, self) => {
    // S'assurer que userId existe et n'est pas null/undefined
    if (!staff.userId) {
      console.warn('Staff without userId:', staff)
      return false
    }
    
    // Trouver le premier index de cet userId (conversion en string pour comparaison)
    const firstIndex = self.findIndex(s => s.userId && String(s.userId) === String(staff.userId))
    const isFirst = index === firstIndex
    
    if (!isFirst) {
      console.log('Filtering duplicate staff:', staff)
    }
    
    return isFirst
  })
  
  console.log('Filtered unique staff:', uniqueStaff)
  return uniqueStaff
})
