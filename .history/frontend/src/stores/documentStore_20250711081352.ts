...
export const useDocumentStore = defineStore('document', () => {
  ...
  const markDocumentAsViewed = async (documentId: string) => {
    try {
      const response = await secureApiCall(`${API_BASE_URL}/documents/${documentId}/view`, {
        method: 'POST',
      })
      ...
      if (response.ok) {
        ...
        const index = documents.value.findIndex(doc => doc.id === documentId)
        if (index !== -1 && documents.value[index].access) {
          documents.value[index].access!.viewedAt = new Date().toISOString()
        }
      }
    } catch (error) {
  ...
  const redirectToYouSignSignature = async (documentId: string) => {
    try {
      const response = await secureApiCall(`${API_BASE_URL}/documents/${documentId}/signature-link`)
      ...
      if (response.ok) {
        ...
        if (data.signatureUrl) {
          ...
          notify.showNotification('Lien de signature non disponible', 'warning')
        }
      } else {
        throw new Error('Erreur lors de la récupération du lien de signature')
      }
    } catch (error) {
      notify.showNotification('Erreur lors de l\'accès à la signature', 'error')
      throw error
    }
  }
