...
// Méthodes
const initGrid = () => {
  console.log('Initializing grid with posts:', posts.value.length);
  
  if (!gridContainer.value) {
    console.log('Grid container not available yet');
    return;
  }

  if (posts.value.length === 0) {
    console.log('No posts available, skipping grid initialization');
    return;
  }

  // Détruire le grid existant
  if (grid) {
    try {
      grid.destroy();
    } catch (error) {
      console.log('Error destroying existing grid:', error);
    }
    grid = null;
  }
...
// Watcher pour réagir aux changements de posts
watch(posts, (newPosts) => {
  console.log('Posts changed, count:', newPosts.length);
...
onMounted(async () => {
  ...
  // Charger les posts en premier
  await blogStore.fetchPosts();
  console.log('Posts loaded:', blogStore.posts.length);
  
  // Configurer les actions globales
  setupGlobalActions();

  // Attendre que Vue.js monte complètement le composant
  await nextTick();
  
  // Initialiser le grid après un petit délai pour s'assurer que tout est prêt
  setTimeout(() => {
    console.log('Attempting to initialize grid after timeout');
    initGrid();
  }, 100);
});
</script>
