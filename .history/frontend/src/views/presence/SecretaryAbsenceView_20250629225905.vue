<template>
  <main class="profile-container" lang="fr" role="main">
    ...
    <div class="skip-links">
      ...
      <a href="#pending-absences" class="skip-link">Aller aux absences non justifiées</a>
    </div>
    ...
    <div class="profile-content" id="main-content">
      <div class="content-grid">
        ...
        <!-- Enfants présents -->
        <div v-if="sheet && presentRecords.length" class="profile-section">
          <div class="section-header">
            <h2>
              <i class="material-icons" aria-hidden="true">check_circle</i>
              Enfants présents
              <span class="child-count">({{ presentCount }})</span>
            </h2>
          </div>
        ...
        <div v-if="sheet && pendingRecords.length" class="profile-section" id="pending-absences">
          <div class="section-header">
            <h2>
              <i class="material-icons" aria-hidden="true">error</i>
              Absents non justifiés
              <span class="child-count">({{ pendingCount }})</span>
            </h2>
            <div class="info-note">
              <i class="material-icons" aria-hidden="true">info</i>
              <span>Cliquez sur "Justifier" pour ajouter une justification</span>
            </div>
          </div>
        ...
        <!-- Retards justifiés -->
        <div v-if="sheet && justifiedLateness.length" class="profile-section">
          <div class="section-header">
            <h2>
              <i class="material-icons" aria-hidden="true">schedule</i>
              Retards justifiés
              <span class="child-count">({{ latenessCount }})</span>
            </h2>
          </div>
        ...
        <!-- Absences justifiées -->
        <div v-if="sheet && justifiedAbsences.length" class="profile-section">
          <div class="section-header">
            <h2>
              <i class="material-icons" aria-hidden="true">check</i>
              Absences justifiées
              <span class="child-count">({{ absenceCount }})</span>
            </h2>
          </div>
            ...
            <table class="custom-table" role="table" aria-label="Liste des absences justifiées">
              ...
              <tbody>
                <tr v-for="rec in justifiedAbsences" :key="rec.id">
                  <td>{{ rec.child.lastName }}</td>
                  <td>{{ rec.child.firstName }}</td>
                  <td>{{ rec.child.parentPhone || 'N/A' }}</td>
                  <td>{{ formatDate(rec.justification!.justificationDate) }}</td>
                  <td>{{ rec.justification!.motif }}</td>
                  <td class="action-cell">
                    <div class="file-actions" v-if="rec.justification!.filePath">
...
<style scoped>
/* ===== Reprendre les styles de StaffPresenceView.vue ===== */
