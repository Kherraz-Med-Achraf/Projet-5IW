...
/** Fonctions utilitaires - adaptée pour détecter les cours sans enfants **/
function getEventColor(event: any): string {
  // Cours annulé si nom contient "annulé" OU pas d'enfants
...
const displayEvents = computed(() => {
  ...
  return events.value.map(event => {
    // Détecter si le cours est annulé (nom contient "annulé" OU pas d'enfants)
    const isCancelled = event.activity.toLowerCase().includes('annulé') || 
                       !event.children || event.children.length === 0
    const isVacation = event.activity.toLowerCase().includes('vacances') || 
                      event.activity.toLowerCase().includes('férié')
...
async function handleCancelCourse(course: any) {
  ...
  if (course.cancelled) {
    try {
      console.log('Réactivation du cours:', course.id)
      await planning.reactivateCourse(course.id)
      console.log('Cours réactivé avec succès')
      await loadEvents()
      closeModal()
    } catch (error) {
