...
import {
  ...
  /**
 * Définit si la semaine contenant `date` est en zone C (vacances scolaires)
 * ou contient un jour férié national.
 * À toi de compléter la logique (JSON, date-holidays, etc.).
 */
private _isVacationWeek(date: Date): boolean {
    // TODO → implémenter : 
    //   • charger un tableau de plages de vacances pour la zone C
    //   • ou utiliser une lib comme `date-holidays`
    //   • retourner true si `date` tombe dedans
    return false;
  }
