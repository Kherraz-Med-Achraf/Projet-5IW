...
onMounted(async () => {
  ...
  // fetchContacts() est déjà appelé dans init(), pas besoin de le refaire
});
...
async function startChat(contactId: string) {
  ...
  try {
    ...
    // Ajouter immédiatement le nouveau chat à la liste locale
    const newChat = {
      id: chatId,
    ...
    // Ajouter le chat en tête de liste
    chatStore.chats.unshift(newChat);
    
    // Rafraîchir la liste des chats après création pour synchroniser avec le serveur
