...
onMounted(async () => {
  ...
  // Afficher un toast le 25 pour le rôle STAFF si journal non rempli
  if (auth.user?.role === 'STAFF') {
    ...
    if (today.getDate() === 25) {
      ...
      // Liste des enfants référents
      const childIds: number[] = auth.user.referentChildren?.map(c => c.id) || []
...
async function toggleOtp() {
  if (otpEnabled.value) {
    ...
    try {
      ...
      toast.error(err.message || 'Erreur lors de l’activation OTP')
      otpEnabled.value = false
    } finally {
      loading.value = false
    }
    ...
    try {
      ...
      toast.error(err.message || 'Erreur lors de la désactivation de l’OTP')
      otpEnabled.value = true
    } finally {
      loading.value = false
    }
