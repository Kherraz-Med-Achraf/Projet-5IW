...
import {
  ...
  export class PlanningService {
    ...
    /**
     * 9. Prévisualisation + validations globales, avec filtre des noms vides
     */
    async previewExcel(
      ...
      dbStaff.forEach(u => {
        ...
        const key = `${u.staffProfile.firstName} ${u.
          staffProfile.lastName}`.toLowerCase();
      ...
      // Initialiser la couverture enfants
      ...
      for (const sheetName of sheetNames) {
        ...
        // Itérer chaque ligne de staff (à partir de la 2ᵉ)
        for (let i = 1; i < rows.length; i++) {
          ...
          for (let idx = 1; idx <= lastIdx; idx++) {
            ...
            // Séparation activité – listes d'enfants
            ...
            // Split & filter pour supprimer les chaînes vides
            let childrenNames = namesPart
              .split(',')
            ...
            // Validation existance des enfants
            ...
            // Marquer la couverture matin/après-midi
            const block = idx <= 3 ? 'morning' : 'afternoon';
            childrenNames.forEach(cn => {
              coverage[dow][cn][block === 'morning' ? 'm' : 'a'] = true;
            });
  
            // Déterminer horaire selon la colonne
            const times = {
              1: ['09:00', '10:00'],
              2: ['10:00', '11:00'],
              3: ['11:00', '12:00'],
      ...
      // 2) Vérifier qu’on a vu tous les staff
      ...
      // 3) Vérifier la couverture des enfants
      const errors: string[] = [];
      for (let dow = 1; dow <= 5; dow++) {
