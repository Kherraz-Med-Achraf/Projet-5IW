...
export const useAuthSecureStore = defineStore('authSecure', {
  ...
  actions: {
    /**
     * Initialisation du store (à appeler au démarrage de l'app)
     */
    async initialize() {
      // Restaurer l'utilisateur depuis le localStorage
      const user = SecureTokenManager.getUser()
      if (user && !SecureTokenManager.isTokenExpired()) {
        this.user = user
    ...
    async login(credentials: { email: string; password: string }) {
      ...
      try {
        // S'assurer d'avoir un token CSRF avant de faire la requête
        await this.ensureCSRFToken()

        // Appel direct à l'endpoint de login
        const data = await SecureHttpClient.json(API_ENDPOINTS.AUTH.LOGIN, {
          method: 'POST',
          body: JSON.stringify(credentials),
        })

        // Si OTP requis (tempToken présent mais pas de user)
        ...
        if (data.user) {
          SecureTokenManager.setAuth(data.user, data.expires_in || 3600)
          this.user = data.user
          ...
          return { 
            success: true, 
            access_token: data.access_token || 'authenticated',
            user: data.user 
          }
        }
    ...
    /**
     * Méthode de compatibilité pour l'ancien store
     */
    ...
    async verifyOtp(otpCode: string) {
      ...
      try {
        ...
        this.error = error.message || 'Code OTP invalide'
        return { success: false, error: this.error }
      } finally {
        this.loading = false
      }
