...
export class AuthService {
  ...
  async sendEmailVerification(userId: string, email: string) {
    ...
    const link =
      ...
      `/auth/confirm-email?uid=${userId}&token=${plain}`;
    const html = `
  ...
  /**
   * Crée un utilisateur avec le rôle forcé (forcedRole).
   * Si forcedRole === Role.PARENT, crée aussi un ParentProfile (+ enfants, + contacts).
   * Sinon, crée simplement la ligne User avec le rôle approprié.
   */
  async registerWithRole(
    ...
    const {
      ...
      children = [],
      emergencyContacts = [],
      ...parentData            // firstName, lastName, phone, address, legalResponsibility, etc.
    } = dto;
    ...
    /* 2. pré-formatage des sous-objets -------------------------------------- */
    // Children (on jette ceux qui ne sont pas remplis correctement)
    const childCreates = children
      .filter(
        (c) =>
          c.firstName?.trim() &&
          c.lastName?.trim() &&
    ...
    // Emergency contacts (même logique)
    const contactCreates = emergencyContacts
      .filter(
        (c) =>
          (c.firstName?.trim() || c.lastName?.trim() || c.phone?.trim())
      )
      .map((c) => ({
    ...
    if (forcedRole === Role.PARENT) {
      return this.prisma.user.create({
        data: {
          ...
          parentProfile: {
            create: {
              ...parentData,                    // firstName, lastName, phone, address, etc.
  
              // blocs optionnels : on ne les met que s’il y a des données valides
              ...(contactCreates.length && {
                emergencyContacts: { create: contactCreates },
              }),
              ...(childCreates.length && {
                children: { create: childCreates },
              }),
            },
          },
        },
    ...
    // autres rôles (pas de sous-objets)
    return this.prisma.user.create({
      data: { email, password: passwordHash, role: forcedRole },
    });
  }
