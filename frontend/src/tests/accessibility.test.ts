...
const router = createRouter({
  ...
  routes: [
    ...
    { path: '/presence', component: { template: '<div>Presence</div>' } },
    { path: '/presence/staff', component: { template: '<div>Staff Presence</div>' } },
...
describe('Accessibility Tests for Home Views', () => {
  let wrapper: any;
  ...
  describe('DIRECTOR Role Accessibility', () => {
    beforeEach(() => {
      // Mock director user
      wrapper.vm.auth.user = { role: 'DIRECTOR', firstName: 'John', lastName: 'Doe' };
    });

    it('should have proper skip links for navigation', async () => {
      await wrapper.vm.$nextTick();
      ...
      skipLinks.forEach((link, index) => {
        expect(link.attributes('href')).toBe(expectedSkipLinks[index]);
      });
    });
    ...
    it('should have proper ARIA labels and roles', async () => {
      ...
      expect(main.attributes('lang')).toBe('fr');
      
      const sections = wrapper.findAll('section');
      sections.forEach(section => {
        expect(section.attributes('class')).toContain('profile-section');
      });
    });
    ...
    it('should have accessible form elements', async () => {
      ...
      const emailInput = wrapper.find('#director-invite-email');
      expect(emailInput.attributes('type')).toBe('email');
      expect(emailInput.attributes('aria-describedby')).toBe('invite-help');
      expect(emailInput.attributes('required')).toBeDefined();
      
      const label = wrapper.find('label[for="director-invite-email"]');
    ...
    it('should have proper button accessibility', async () => {
      ...
      const buttons = wrapper.findAll('button');
      buttons.forEach(button => {
        // Check if button has either text content or aria-label
    ...
    it('should have proper heading hierarchy', async () => {
      ...
      const h1 = wrapper.find('h1'); // From PageHeader
      expect(h1.exists()).toBe(true);
      
      const h2s = wrapper.findAll('h2');
      expect(h2s.length).toBeGreaterThan(0);
      
      // Check that section headers are properly structured
      h2s.forEach(h2 => {
        expect(h2.closest('.section-header')).toBeTruthy();
      });
    });

    it('should have proper live regions for messages', async () => {
      await wrapper.vm.$nextTick();
      
      // Set up invitation message
      await wrapper.setData({ inviteMessage: 'Test message', inviteError: false });
  ...
  describe('SERVICE_MANAGER Role Accessibility', () => {
    beforeEach(() => {
      // Mock service manager user
      wrapper.vm.auth.user = { role: 'SERVICE_MANAGER', firstName: 'Jane', lastName: 'Smith' };
    });
    ...
    it('should have unique form IDs for service manager', async () => {
      await wrapper.vm.$nextTick();
      
      const emailInput = wrapper.find('#manager-invite-email');
      expect(emailInput.attributes('type')).toBe('email');
      expect(emailInput.attributes('aria-describedby')).toBe('invite-help-manager');
      
      const label = wrapper.find('label[for="manager-invite-email"]');
      expect(label.exists()).toBe(true);
    });
  ...
  describe('SECRETARY Role Accessibility', () => {
    beforeEach(() => {
      // Mock secretary user
      wrapper.vm.auth.user = { role: 'SECRETARY', firstName: 'Marie', lastName: 'Dupont' };
    });
    ...
    it('should have accessible statistics display', async () => {
      await wrapper.vm.$nextTick();
  ...
  describe('Keyboard Navigation', () => {
    it('should be navigable with keyboard', async () => {
      ...
      const focusableElements = wrapper.findAll('button, a, input, [tabindex]');
      
      focusableElements.forEach(element => {
        // Check that focusable elements are not hidden
        expect(element.attributes('tabindex')).not.toBe('-1');
        
        // Check that disabled elements have proper attributes
  ...
  describe('Screen Reader Compatibility', () => {
    it('should have proper ARIA attributes', async () => {
      ...
      const loadingIndicators = wrapper.findAll('.loading-indicator');
      loadingIndicators.forEach(indicator => {
        const spinningIcon = indicator.find('.spinning');
        if (spinningIcon.exists()) {
          expect(spinningIcon.attributes('aria-hidden')).toBe('true');
        }
      });
    });
    ...
    it('should have proper status messages', async () => {
      await wrapper.vm.$nextTick();
  ...
  describe('Color Contrast and Visual Accessibility', () => {
    it('should have proper color contrast for text', async () => {
      ...
      const textElements = wrapper.findAll('h1, h2, h3, h4, p, span, label');
      textElements.forEach(element => {
        const styles = window.getComputedStyle(element.element);
        const backgroundColor = styles.backgroundColor;
        const color = styles.color;
        
        // Basic check that text has both background and foreground colors
...
/**
 * Test helper function to check VoiceOver compatibility
 * This function can be used in manual testing or with automation tools
 */
export function testVoiceOverCompatibility() {
  const accessibilityChecks = {
    skipLinks: () => {
      const skipLinks = document.querySelectorAll('.skip-link');
      return Array.from(skipLinks).every(link => {
    ...
    focusableElements: () => {
      ...
      return Array.from(focusable).every(element => {
        return element.getAttribute('tabindex') !== '-1';
      });
    }
  };
  
  const results = Object.entries(accessibilityChecks).map(([name, check]) => ({
    name,
    passed: check(),
  }));
  
  console.log('VoiceOver Compatibility Check Results:', results);
  return results;
}

/**
 * Test helper function to simulate keyboard navigation
 */
export function testKeyboardNavigation() {
  ...
  const navigate = (direction: 'forward' | 'backward') => {
    ...
    console.log(`Focused element: ${element.tagName} ${element.className}`, element);
    return element;
  };
