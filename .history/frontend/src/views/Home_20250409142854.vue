...
// Au montage, on attend que le store soit hydraté, puis on ajuste l'état de la checkbox
onMounted(async () => {
  await auth.hydrate()
...
// Optionnel : Observe auth.user avec flush 'post' pour s'assurer d'avoir les dernières mises à jour
watch(
  () => auth.user,
  async (newUser) => {
...
async function toggleOtp() {
  if (otpEnabled.value) {
    // On active l'OTP si la case est cochée
    loading.value = true
    try {
    ...
    // On désactive l'OTP si la case est décochée
    loading.value = true
    try {
