import {
  ...
  export class PlanningService {
     ...
     */
    private _getVacations(year: number): any[] {
      const anyHd = hd as any;
      if (typeof anyHd.getVacations === 'function') {
        return anyHd.getVacations(year) ?? [];
      }
      // Fallback : on utilise getHolidays et on filtre les entrées de type "school"
      if (typeof anyHd.getHolidays === 'function') {
        const hols = anyHd.getHolidays(year) ?? [];
        return hols
          .filter((h: any) => h.type === 'school')
          .map((h: any) => ({
            // Normalise les propriétés attendues par l'appelant
            start: this._atNoon(new Date(h.start ?? h.date)),
            end:   this._atNoon(new Date(h.end   ?? h.date)),
            // certains jeux de données utilisent schoolZone / zones / subregion
            schoolZone: (h.schoolZone ?? h.zones ?? h.subregion ?? '').toString(),
            name: h.name ?? '',
          }));
      }
      return [];
    ...
    private _getClosureLabel(date: Date): string | null {
      ...
      const vacations: any[] = this._getVacations(year);
      const inZoneParis = vacations.find(vac => {
        const zoneRaw = (vac.schoolZone ?? vac.zones ?? vac.subregion ?? '').toString();
        const zone = zoneRaw.toUpperCase();
        const name = (vac.name ?? '').toString().toLowerCase();

        // Zone C (Paris, Versailles, Créteil, Montpellier, Toulouse) ou mention explicite Paris/Île-de-France
        const isParisZone = zone.includes('C') || zone.includes('PARIS') || zone.includes('ILE') || zone.includes('ÎLE');
        const nameHints   = name.includes('zone c') || name.includes('paris') || name.includes('île-de-france') || name.includes('ile-de-france');

        if (!(isParisZone || nameHints)) return false;
        const start = this._atNoon(new Date(vac.start));
        const end   = this._atNoon(new Date(vac.end));
        return dNoon >= start && dNoon <= end;
      });
      if (inZoneParis) return 'Vacances scolaire';
    ...
    async importExcel(
      ...
      await this.prisma.$transaction(
        async tx => {
          // on supprime d'abord les anciennes données du semestre
