...
export class PresenceService {
  ...
  async createSheet(dateString: string, staffId: string) {
    ...
    // 3) Retourne la feuille complète avec les enregistrements, l’enfant, la justification et le profil du staff
  ...
  async validateSheet(
    ...
    // Recréation de tous les enregistrements avec le flag présent/absent
    const children = await this.prisma.child.findMany();
    ...
    // Passe la feuille au statut suivant et enregistre qui l’a validée
  ...
  async justify(recordId: number, dto: JustifyAbsenceDto, filePath?: string) {
    ...
    // Détermine un motif par défaut si LATENESS
    const motifValue =
      dto.motif ??
      (dto.type === JustificationType.LATENESS
        ? 'Retard justifié'
        : '');

    // Création de la justification avec le type (ABSENCE / LATENESS)
    ...
    // Retourne la feuille mise à jour
    return this.prisma.presenceSheet.update({
