...
async function main() {
  ...
  // Générer les journaux mensuels
  const currentDate = new Date();
  const currentYear = currentDate.getFullYear();
  const currentMonth = currentDate.getMonth() + 1; // getMonth() retourne 0-11
  
  // Période : de janvier de l'année précédente jusqu'au mois précédent le mois actuel
  const startYear = currentYear - 1;
  const endYear = currentYear;
  const endMonth = currentMonth - 1 === 0 ? 12 : currentMonth - 1;
  const actualEndYear = currentMonth - 1 === 0 ? currentYear - 1 : currentYear;
  
  console.log(`ℹ️  Génération des journaux de janvier ${startYear} à ${endMonth === 12 ? 'décembre' : `mois ${endMonth}`} ${actualEndYear}`);
  ...
  if (allEducators.length === 0) {
    ...
    for (const child of allChildren) {
      ...
      // Générer pour chaque mois de la période
      for (let year = startYear; year <= actualEndYear; year++) {
        const startMonthForYear = year === startYear ? 1 : 1;
        const endMonthForYear = year === actualEndYear ? endMonth : 12;
        
        // Trouver l'année scolaire correspondante
        const academicYear = academicYears.find(ay => {
          const [ayStartYear] = ay.label.split('-').map(Number);
          return (year >= ayStartYear && year <= ayStartYear + 1);
        });
        
        if (!academicYear) {
          console.log(`⚠️  Année scolaire non trouvée pour ${year}`);
          continue;
        }
    ...
    console.log('✅ Journaux mensuels créés pour tous les enfants');
  }
}
