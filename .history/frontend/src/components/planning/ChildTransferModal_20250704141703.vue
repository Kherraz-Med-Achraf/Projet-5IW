    ...
    >
      <div class="modal-header">
        ...
        <button 
          @click="closeModal" 
          class="close-btn" 
          aria-label="Fermer la modale de transfert des enfants"
          ref="closeButton"
        >
      ...
      <div class="modal-body" id="modal-description">
        <div class="cancelled-course-info">
          ...
          <div class="course-card cancelled" role="group" aria-labelledby="cancelled-course-title">
            <div class="course-title" id="cancelled-course-title">{{ cancelledCourse.title }}</div>
        ...
        <div class="transfer-section" v-if="childrenToTransfer.length > 0">
          ...
          <div class="children-list" role="list" aria-label="Liste des enfants à transférer">
            <div
            ...
            >
              ...
              <div class="transfer-options">
                <label 
                  :for="`course-select-${child.id}`" 
                  class="transfer-label"
                >
                  Nouveau cours pour {{ child.firstName }} {{ child.lastName }} :
                </label>
                <select 
                  :id="`course-select-${child.id}`"
                  v-model="transfers[child.id]"
                  class="course-select"
                  @change="updateTransfer(child.id, $event.target.value)"
                  :aria-describedby="`course-help-${child.id}`"
                  :aria-invalid="!transfers[child.id] && childrenToTransfer.length > 0 ? 'true' : 'false'"
                >
                ...
                <small 
                  :id="`course-help-${child.id}`" 
                  class="course-help"
                  :aria-live="transfers[child.id] ? 'polite' : 'off'"
                >
                  {{ transfers[child.id] ? 'Cours sélectionné' : 'Sélectionnez un cours de destination' }}
                </small>
              </div>
            </div>
          </div>

          <div v-if="alternatives.length === 0" class="no-alternatives" role="alert" aria-live="polite">
            <i class="material-icons" aria-hidden="true">warning</i>
            <div>
              <p><strong>Aucun cours alternatif disponible au même horaire.</strong></p>
              <p class="help-text">Les enfants resteront associés au cours annulé.</p>
            </div>
          </div>
        </div>

        <div v-else class="no-children" role="status" aria-live="polite">
      ...
      <div class="modal-footer">
        <button 
          ...
          aria-label="Annuler le transfert des enfants"
        >
          <i class="material-icons" aria-hidden="true">close</i>
          Annuler
        </button>
        ...
        <button 
          ...
          :aria-label="allChildrenAssigned ? 'Confirmer le transfert des enfants' : 'Impossible de confirmer, tous les enfants doivent avoir un cours assigné'"
          :aria-describedby="!allChildrenAssigned ? 'transfer-warning' : undefined"
        >
        ...
        <div 
          v-if="!allChildrenAssigned && childrenToTransfer.length > 0" 
          id="transfer-warning" 
          class="transfer-warning"
          role="alert"
          aria-live="polite"
        >
          <small>Tous les enfants doivent avoir un cours assigné</small>
        </div>
      </div>
    </div>
  </div>
...
.modal-footer {
  ...
  .btn {
    ...
    &.btn-primary {
      ...
      &:disabled {
        background: #9ca3af; /* Couleur plus foncée pour meilleur contraste */
        color: #ffffff;
        cursor: not-allowed;
        transform: none;
        box-shadow: none;
