...
export class ChildService {
  ...
  async createForParent(parentProfileId: number, dto: CreateChildDto): Promise<Child> {
    ...
    return this.prisma.$transaction(async tx => {
      ...
      // génération login/mdp et création user…
  ...
  async updateForParent(
    ...
    // transaction pour MAJ + potentielle mise à jour du login + mail
  ...
  async removeForParent(parentProfileId: number, id: number): Promise<{ id: number }> {
    ...
    // prévenir le parent de la suppression
  ...
  /*** MÉTHODES GÉNÉRIQUES POUR LES STAFF/ADMINS ***/
