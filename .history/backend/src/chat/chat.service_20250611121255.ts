...
import {
  ...
  import {
    Chat,
    ChatDocument,           // <- type avec participants: string[]
  ...
  export class ChatService {
    ...
    async createMessage(chatId: string, authorId: string, content: string) {
      ...
      return this.msgModel.create({
        ...
        author: authorId,                 // id Postgres brut
        content,
      });
    }
      ...
      switch (role) {
        ...
        case 'SERVICE_MANAGER':
          return this.prisma.user
            ...
            .then((u) =>
              u.map((m) => ({ id: m.id, name: m.email, role: m.role })),
            );
    ...
    private async getStaffContacts(staffId: string): Promise<Contact[]> {
      ...
      const uniqueParents = Array.from(new Map(
