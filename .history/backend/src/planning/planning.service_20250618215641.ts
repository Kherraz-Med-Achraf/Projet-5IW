import {
  ...
  export class PlanningService {
    ...
    private async _expandEntriesOverSemester(
        ...
        for (let w = 0; w < totalWeeks; w++) {
          ...
          for (const be of baseEntries) {
            // Construit la date exacte du créneau sur cette semaine
            const entryDate = addDays(weekMonday, be.dayOfWeek - 1);
      
            // Si c'est un jour férié ou dans les vacances zone C, on saute ce créneau
            if (this._isNonTeachingDay(entryDate)) continue;
    ...
    /** Détermine si une date est fériée OU située dans une période de vacances scolaires (zone C) */
    private _isNonTeachingDay(date: Date): boolean {
      // 1) Jour férié national
      if (hd.isHoliday(date)) return true;
      
      // 2) Vacances scolaires (zone C / Paris)
      const year = date.getFullYear();
      const vacations: any[] = (hd as any).getVacations(year) ?? [];
      
      // Filtre uniquement la zone C (selon lib : schoolZone | zones | subregion | name)
      const zoneCVac = vacations.filter(vac => {
        const zone = (vac.schoolZone ?? vac.zones ?? vac.subregion ?? '').toString().toUpperCase();
        return zone.includes('C') || (vac.name && vac.name.toLowerCase().includes('zone c'));
      });
      
      return zoneCVac.some(vac => {
