...
async function onUpload() {
  ...
  try {
    ...
    // Vérifier si le nom a été modifié pour informer l'utilisateur
    const originalName = selectedFile.value.name;
    const sanitizedName = att.filepath;
    
    selectedFile.value = null;
    
    if (originalName !== sanitizedName) {
      toast.success(`Fichier ajouté avec succès. Nom modifié pour éviter les caractères spéciaux : "${sanitizedName}"`);
    } else {
      toast.success(`Fichier "${sanitizedName}" ajouté avec succès`);
    }
  } catch (e: any) {
