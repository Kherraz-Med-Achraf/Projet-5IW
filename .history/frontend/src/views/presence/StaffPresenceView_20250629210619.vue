...
function initGrid() {
  ...
  console.log('GridJS data prepared:', data)

  // Define columns based on validation state - chaque colonne correspond à un index des données
  const columns = [
    { 
      name: 'Nom', 
      width: '25%'
    ...
    },
    {
      ...
      hidden: true
      // data[4] = justificationType
    },
    {
      name: 'justificationMotif', 
      hidden: true
      // data[5] = justificationMotif
    },
    {
      name: 'Présent',
      width: '15%',
      // data[3] = present, mais cette colonne sera à l'index 6 maintenant
      formatter: (cell: any, row: any) => {
        // cell = valeur de cette colonne (present)
        const isPresent = cell
        const childId = row.cells[2]?.data  // colonne childId (index 2)
        const justificationType = row.cells[4]?.data  // colonne justification type (index 4)
        
        console.log('Presence formatter:', { childId, isPresent, justificationType })
        
        if (childId === undefined || childId === null) {
          console.error('childId is missing:', childId)
          return html(`<span>ID manquant</span>`)
        }
    ...
    columns.push({
      ...
      formatter: (cell: any, row: any) => {
        ...
        if (row && row.cells && Array.isArray(row.cells)) {
          type = row.cells[4]?.data
          motif = row.cells[5]?.data
