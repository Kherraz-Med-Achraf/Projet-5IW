...
<template>
  <div class="planning-upload">
    ...
    <!-- 2bis) Saisie des semaines de vacances -->
    <div class="form-group">
      <label for="vacWeeks">Semaines de vacances (ex&nbsp;: 2025-W07,2025-W08)</label>
      <input
        id="vacWeeks"
        type="text"
        placeholder="AAAA-Www,AAAA-Www…"
        v-model="vacationWeeksStr"
      />
    </div>
...
const vacationWeeksStr   = ref<string>('')
const vacationPeriods    = ref<{ start: string; end: string }[]>([])
...
const vacationEvents = computed(() =>
  vacationPeriods.value.map((p, idx) => ({
    id: `vac-${idx}`,
    title: 'Vacances scolaires',
    start: p.start,
    end:   formatISO(new Date(new Date(p.end).getTime() + 24*60*60*1000)), // inclusif
    allDay: true,
    color: '#FFB74D',
  }))
)
...
const calendarEvents = computed(() => {
  const events = previewEntries.value
    .filter(e => e.staffId === selectedStaffId.value && !isVacation(e.startTime))
    .map(e => ({
      id: e.id,
      title: e.activity,
      start: e.startTime,
  ...
  return [...events, ...vacationEvents.value]
})
...
/** Charge les vacances scolaires zone C pour l'intervalle du semestre */
async function loadVacations() {
  if (!planning.selectedSemester) return
  const sem = planning.selectedSemester
  const from = sem.startDate.split('T')[0]
  const to   = sem.endDate.split('T')[0]
  const url = `https://openholidaysapi.org/SchoolHolidays?countryIsoCode=FR&subdivisionCode=FR-C&languageIsoCode=FR&validFrom=${from}&validTo=${to}`
  try {
    const res = await fetch(url)
    if (!res.ok) throw new Error('Erreur chargement vacances')
    const data = await res.json()
    vacationPeriods.value = (data || []).map((p: any) => ({
...
/** Teste si une date ISO est dans une période de vacances */
function isVacation(iso: string) {
  const d = iso.substring(0, 10)
  return vacationPeriods.value.some(p => d >= p.start && d <= p.end)
}
