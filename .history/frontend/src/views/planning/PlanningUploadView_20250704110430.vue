  ...
  <main class="profile-container" role="main">
    ...
    <div class="profile-content" id="main-content">
      <div class="content-grid">
          ...
          <div class="header-content">
            <div class="title-section">
              ...
              <div class="title-info">
                ...
                <p class="subtitle">Import et gestion des emplois du temps pour tous les éducateurs</p>
              </div>
            </div>
            <div v-if="selectedSemester" class="status-indicator">
        ...
        <section class="profile-section" id="semester-selection" aria-labelledby="config-title">
          ...
          <div class="config-grid">
            ...
            <div class="config-item">
              ...
              <small class="config-help">Le planning sera appliqué à ce semestre</small>
            </div>
            ...
            <div class="config-item" id="file-upload">
              ...
              <div v-if="hasExistingPlanning" class="existing-planning-info">
                ...
                <details class="replace-planning">
                  <summary>Remplacer le planning existant</summary>
              ...
              <small class="config-help">Format attendu : 5 onglets (Lundi à Vendredi) avec éducateurs en lignes</small>
            </div>
          </div>
          ...
          <div class="action-section">
            <div class="action-buttons">
              <button 
                @click="previewPlanning" 
                :disabled="!canPreview || loading"
                class="btn btn-primary"
              >
                <i class="material-icons">visibility</i>
                {{ loading ? 'Traitement...' : 'Prévisualiser' }}
              </button>
        ...
        <!-- Sélection éducateur -->
        <section v-if="previewEntries.length" class="profile-section" aria-labelledby="staff-title">
          <div class="section-header">
            <h2 id="staff-title">
...
const calendarEvents = computed(() => {
  const raw = previewEntries.value
    ...
    .map(e => {
      ...
      // Pour les vacances/fériés, étendre de 8h à 16h pour plus de visibilité
      ...
      if (isVacation) {
        ...
        const startDate = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 8, 0, 0)
        const endDate = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 16, 0, 0)
...
const calendarOptions = ref({
  ...
  eventContent(info: any) {
    if (info.view.type === 'dayGridMonth') {
      ...
      else if (act.includes('vacances')) label = 'Vacances scolaires'
      else if (act.startsWith('annulé –')) label = 'Cours annulé'
      
      return { html: `<div class="calendar-event-label">${label}</div>` }
    }
    return true
  },
  eventClick(info: any) {
    const kids = info.event.extendedProps.children || []
    // Pour les vacances/fériés, utiliser les horaires originaux, sinon les horaires affichés
    const useOriginalTimes = info.event.extendedProps.vacation && 
                            info.event.extendedProps.originalStart && 
                            info.event.extendedProps.originalEnd
    
    selectedCourse.value = {
      title: info.event.title,
      start: useOriginalTimes ? new Date(info.event.extendedProps.originalStart) : info.event.start,
      end: useOriginalTimes ? new Date(info.event.extendedProps.originalEnd) : info.event.end,
      children: kids,
