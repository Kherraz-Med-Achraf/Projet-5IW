...
// Mock date-holidays
const mockHolidays = jest.fn().mockImplementation(() => ({
  getHolidays: jest.fn().mockReturnValue([]),
}));
jest.mock('date-holidays', () => mockHolidays);
...
describe('PlanningService', () => {
  ...
  describe('getSemesters', () => {
    ...
    it('should return empty array when no semesters', async () => {
      ...
      const result = await service.getSemesters();
  ...
  describe('getScheduleOverview', () => {
      ...
      const entries = [
        {
          ...
          staff: { staffProfile: mockStaff[0].staffProfile },
        },
      ];
      mockPrisma.scheduleEntry.findMany.mockResolvedValue(entries);

      const result = await service.getScheduleOverview('sem-1');
    ...
    it('should return empty array for semester with no entries', async () => {
      ...
      const result = await service.getScheduleOverview('empty-sem');
  ...
  describe('previewExcel', () => {
    ...
    beforeEach(() => {
      ...
      mockXLSX.utils.sheet_to_json.mockReturnValue([
        ['Jean Dupont', 'Mathématiques Alice Smith', 'Sciences Bob Johnson'],
      ]);
    ...
    it('should preview Excel import successfully', async () => {
      const result = await service.previewExcel(mockFile, 'sem-1');

      expect(mockXLSX.read).toHaveBeenCalledWith(mockFile.buffer, { type: 'buffer' });
    ...
    it('should throw error for unknown staff member', async () => {
      mockXLSX.utils.sheet_to_json.mockReturnValue([
        ['Unknown Staff', 'Activity'],
      ]);

      await expect(
        service.previewExcel(mockFile, 'sem-1')
      ).rejects.toThrow(BadRequestException);
    });

    it('should validate complete coverage for all children', async () => {
      // Mock incomplete coverage scenario
      mockXLSX.utils.sheet_to_json.mockReturnValue([
        ['Jean Dupont', 'Mathématiques'], // Missing children assignment
      ]);
    ...
    it('should handle Excel with missing sheets', async () => {
      mockXLSX.read.mockReturnValue({
        SheetNames: ['Lundi'], // Missing other weekdays
        Sheets: { Lundi: {} },
      });
  ...
  describe('importExcel', () => {
    ...
    beforeEach(() => {
      ...
      jest.spyOn(service, 'previewExcel').mockResolvedValue([
        {
          id: 'entry-1',
          staffId: 'staff-1',
          semesterId: 'sem-1',
    ...
    it('should import Excel successfully', async () => {
      await service.importExcel(mockFile, 'sem-1');

      expect(service.previewExcel).toHaveBeenCalledWith(mockFile, 'sem-1');
      expect(prisma.$transaction).toHaveBeenCalled();
      expect(mockFs.writeFile).toHaveBeenCalled();
    });

    it('should clean existing entries before import', async () => {
      await service.importExcel(mockFile, 'sem-1');

      expect(prisma.entryChild.deleteMany).toHaveBeenCalledWith({
        where: { entry: { semesterId: 'sem-1' } },
      });
    ...
    it('should handle transaction failures', async () => {
      ...
      await expect(
        service.importExcel(mockFile, 'sem-1')
      ).rejects.toThrow('Transaction failed');

      // File should not be saved if transaction fails
  ...
  describe('getImportedExcelBuffer', () => {
    it('should return Excel file buffer', async () => {
      ...
      const result = await service.getImportedExcelBuffer('sem-1');

      expect(mockFs.readFile).toHaveBeenCalledWith(
        expect.stringContaining('sem-1.xlsx')
      );
    ...
    it('should throw not found for missing file', async () => {
      ...
      await expect(
        service.getImportedExcelBuffer('non-existent')
      ).rejects.toThrow(NotFoundException);
    });
  });

  describe('vacation handling', () => {
    it('should load vacation data for semester period', async () => {
      mockPrisma.semester.findUnique.mockResolvedValue(mockSemester);

      // This method is called internally during preview/import
      const result = await service.previewExcel(
        { buffer: Buffer.from('test') } as Express.Multer.File,
        'sem-1'
      );

      // Verify vacation data was considered
      expect(mockHolidays).toHaveBeenCalledWith('FR', 'fr', 'Europe/Paris');
    });
  });
