...
export class BlogService {
  ...
  /**
   * Créer un nouveau post (Secrétaire uniquement)
   */
  async createPost(dto: CreateBlogPostDto, authorId: string): Promise<BlogPostResponseDto> {
    ...
    if (dto.improveTitleWithAI && title) {
      try {
        title = await this.aiService.improveMission(`Améliore ce titre de post pour un blog d'école spécialisée : "${title}"`);
      } catch (error) {
        console.error('[Blog] Erreur amélioration titre IA:', error);
        // Continue avec le titre original
      }
    }

    if (dto.improveDescriptionWithAI && description) {
      try {
        description = await this.aiService.improveMission(`Améliore cette description de post pour un blog d'école spécialisée : "${description}"`);
      } catch (error) {
        console.error('[Blog] Erreur amélioration description IA:', error);
        // Continue avec la description originale
      }
    }
    ...
    return this.formatPostResponse(post, null);
  }
  ...
  private formatPostResponse(post: any, userReaction?: ReactionType): BlogPostResponseDto {
    ...
    return {
      ...
      author: {
        id: post.author.id,
        firstName: post.author.secretaryProfile?.firstName || 'Auteur',
        lastName: post.author.secretaryProfile?.lastName || 'Inconnu',
      },
