...
// Fonction pour générer le QR code à partir d'un otpSecret existant
...
// Surveille auth.user pour mettre à jour l'état OTP
watch(
  () => auth.user,
  (newUser) => {
...
async function toggleOtp() {
  if (otpEnabled.value) {
    // Si l'utilisateur coche la case et que l'OTP n'est pas déjà activé, on l'active
    if (!auth.user || auth.user.otpSecret) {
      // Si l'OTP est déjà activé, on ne refait rien
      return
    }
    loading.value = true
    try {
    ...
    // Si la checkbox est décochée, on désactive l'OTP
    loading.value = true
    try {
...
.modal {
  /* styles pour une modale si besoin */
}
</style>
