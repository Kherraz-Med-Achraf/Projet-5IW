        ...
        <div class="profile-section" id="personal-info">
          <div class="section-header">
            ...
            <div class="header-actions">
              ...
              <button 
                class="edit-btn test-btn" 
                @click="testNavigation()" 
                type="button"
                style="margin-left: 10px; background: #dc2626;"
              >
                <i class="material-icons" aria-hidden="true">bug_report</i>
                Test Navigation
              </button>
            </div>
          </div>
...
async function openEditModal() {
  try {
    // S'assurer d'avoir un token CSRF avant la requête
    await auth.ensureCSRFToken();
    
    // Charger les données d'édition non masquées
    const response = await SecureHttpClient.request(API_ENDPOINTS.AUTH.PROFILE_EDIT);
...
async function toggleOtp() {
  ...
  try {
    ...
    if (!otpEnabled.value) {
      // Activer l'OTP - utiliser SecureHttpClient
      const response = await SecureHttpClient.request(API_ENDPOINTS.AUTH.ENABLE_OTP, {
        method: 'POST'
      });
      ...
      // Désactiver l'OTP - utiliser SecureHttpClient
      const response = await SecureHttpClient.request(API_ENDPOINTS.AUTH.DISABLE_OTP, {
        method: 'POST'
      });
...
function testNavigation() {
  ...
  // Add any additional logic you want to execute when the test button is clicked
}
</script>
