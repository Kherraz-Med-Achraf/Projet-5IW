...
import {
  ...
  export class PlanningService {
     ...
     *    • Colonnes B→F : créneaux (B-D matin, E-F après-midi; mercredi : B-D)
     *    • Cellule = "Activité – Prénom Nom, Prénom Nom…"
     *    • Validation :
     *      – Tous les STAFF du fichier sont dans la base (insensible à la casse)
     *      – Tous les STAFF en base sont listés dans le fichier
     *      – Tous les ENFANTS en base apparaissent (insensible à la casse)
     *      – Pour chaque jour et chaque enfant : créneau matin + après-midi (mercredi : matin uniquement)
     */
    async previewExcel(
      ...
      // Charger tous les éducateurs en base
      ...
      for (const sheetName of sheetNames) {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          for (let idx = 1; idx <= lastIdx; idx++) {
            ...
            if (!act || !names) {
              throw new BadRequestException(
                `Format invalide [Activité – Enfants] dans ${sheetName}, ligne ${i + 1}, colonne ${idx + 1}`,
              );
            }
            ...
            childrenNames.forEach(cn => {
              if (!childSet.has(cn)) {
                throw new BadRequestException(`Enfant introuvable en base : "${cn}"`);
              }
            });
            // Déterminer matin/après-midi
            ...
            allEntries.push({
              ...
              children: dbChildren
                .filter(c =>
                  childrenNames.includes(`${c.firstName} ${c.lastName}`.toLowerCase()),
                )
                .map(c => ({
                  id: c.id,
      ...
      // 2) Vérifier qu’on a vu TOUS les staff en base
      ...
      for (let dow = 1; dow <= 5; dow++) {
        childKeys.forEach(cn => {
          ...
          if (!cov.m) {
            errors.push(`Enfant ${cn} sans créneau matin le ${['','Lundi','Mardi','Mercredi','Jeudi','Vendredi'][dow]}`);
          }
          if (dow !== 3 && !cov.a) {
            errors.push(`Enfant ${cn} sans créneau après-midi le ${['','Lundi','Mardi','Mercredi','Jeudi','Vendredi'][dow]}`);
          }
        });
      }
      if (errors.length) {
