  ...
  actions: {
    ...
    async fetchStaffList() {
      ...
      try {
        ...
        // Déduplication au niveau du store par précaution
        const uniqueStaff = mappedStaff.filter((staff, index, self) => {
          if (!staff.userId) return false
          return index === self.findIndex(s => s.userId && String(s.userId) === String(staff.userId))
        })
        
        this.staffList = uniqueStaff
        
        console.log('Staff list after mapping and deduplication:', this.staffList.length, 'items')
