...
export const useJournalStore = defineStore('journal', {
  ...
  actions: {
    ...
    /* --------------- IA – suggestion mission ---------- */
    /**
     * Retourne une version améliorée de la mission passée.
     * Le back doit exposer POST /ai/mission-improve
     * body: { prompt: string }  →  { suggestion: string }
     */
    async proposeMissionImprovement(prompt: string): Promise<string> {
      try {
        ...
        // on accepte soit data.suggestion soit data.content
        return (data.suggestion ?? data.content ?? '').toString()
      } catch (e: any) {
        // on propage pour être capturé côté vue
        throw new Error(e.message || 'Erreur IA')
      }
    },
