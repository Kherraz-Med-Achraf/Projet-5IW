...
function initGrid() {
  ...
  setTimeout(() => {
    const searchContainer = gridContainer.value?.querySelector('.gridjs-search')
...
/* Icône de loupe avec Material Icons */
:deep(.gridjs-search)::before {
  content: 'search';
...
:deep(.gridjs-search input:focus) + :deep(.gridjs-search)::before,
:deep(.gridjs-search input:focus) ~ :deep(.gridjs-search)::before {
  color: #4338ca;
}
