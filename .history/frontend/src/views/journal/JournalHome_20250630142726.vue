...
async function onChildChange() {
  if (selectedChildId.value) {
    ...
    loading.value = true;
    try {
      await journalStore.loadAcademicYears();
    } catch (err) {
...
async function onYearChange() {
  if (selectedYearId.value && selectedChildId.value) {
    ...
    try {
      await Promise.all([
        journalStore.loadMissions(selectedChildId.value, selectedYearId.value),
        journalStore.loadJournals(selectedChildId.value, selectedYearId.value)
      ]);
    } catch (err) {
...
onMounted(async () => {
  ...
  try {
    ...
    await journalStore.loadReferentChildren();
