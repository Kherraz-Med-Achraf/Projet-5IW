...
export class PlanningService {
  ...
  async restoreTransferredChildren(originalEntryId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      // Trouver tous les enfants qui ont été transférés depuis ce cours
      const transferredChildren = await tx.entryChild.findMany({
        where: { originalEntryId: originalEntryId },
        include: { child: true },
      });

      console.log('[restoreTransferredChildren] found', transferredChildren.length, 'children');

      // Les remettre dans le cours original
      for (const transfer of transferredChildren) {
        // Supprimer de leur position actuelle
        await tx.entryChild.delete({
          where: {
            entryId_childId: {
        ...
        // Les remettre dans le cours original (sans originalEntryId cette fois)
        await tx.entryChild.create({
          data: {
            entryId: originalEntryId,
            childId: transfer.childId,
            originalEntryId: null, // Retour à la normale
          },
        });
      }
  ...
  async reassignSingleChild(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Détermine l'ID du cours original (pour la traçabilité)
      ...
      // Crée le nouveau lien avec traçabilité de l'origine
      await tx.entryChild.create({ 
        data: { 
