...
export class YousignService {
  ...
  /* URLs & secrets (env ⇒ .env) */
  private readonly base = process.env.YOUSIGN_API_URL ?? 'https://staging-api.yousign.com/v2';
  private readonly key  = process.env.YOUSIGN_API_KEY!;                    // nouvelle clé
  private readonly hook = process.env.YOUSIGN_WEBHOOK_SECRET ?? '';

  /* -------------------------------------------------- *
   * 1️⃣  Upload du PDF  (v2 → JSON base64)
   * -------------------------------------------------- */
  ...
  /* -------------------------------------------------- *
   * 2️⃣  Création de la procédure + membres
   * -------------------------------------------------- */
  ...
  /* -------------------------------------------------- *
   * 3️⃣  URL signature secrétaire / parent
   * -------------------------------------------------- */
  ...
  /* -------------------------------------------------- *
   * 4️⃣  Téléchargement du PDF final signé
   * -------------------------------------------------- */
  ...
  /* -------------------------------------------------- *
   * 5️⃣  Vérification signature HMAC webhook
   * -------------------------------------------------- */
