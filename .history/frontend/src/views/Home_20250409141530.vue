...
// Fonction pour générer le QR code à partir d'un otpSecret
...
// Au montage, on hydrate le store et on met à jour la checkbox
onMounted(async () => {
  // S'assurer que le store charge les données de l'utilisateur depuis le localStorage
  await auth.hydrate()
  if (auth.user && auth.user.otpSecret) {
    otpEnabled.value = true
    secret.value = auth.user.otpSecret
    await generateQRCodeFromSecret(auth.user.otpSecret)
  } else {
    otpEnabled.value = false
  }
})

// Optionnel: on observe auth.user pour mettre à jour au cas où il change
watch(
  () => auth.user,
  async (newUser) => {
...
async function toggleOtp() {
  if (otpEnabled.value) {
    ...
    // La case est décochée (désactivation)
    loading.value = true
    try {
