    ...
    columns: [
      {
        ...
        formatter: (_, row) => {
          ...
          console.log('GridJS - Image formatter - post complet:', JSON.stringify(post, null, 2));
          
          if (post?.mediaUrl) {
            // Construire l'URL complète pour l'image
            const fullImageUrl = post.mediaUrl.startsWith('http') 
              ? post.mediaUrl 
              : `${API_BASE_URL}${post.mediaUrl}`;
              
            return html(`
              <img src="${fullImageUrl}" alt="Image du post" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px" onerror="this.style.display='none'">
            `);
          } else {
            return html(`
      ...
      },
      {
        ...
        formatter: (_, row) => {
          ...
          const totalReactions = Object.values(post.reactions).reduce((total, count) => total + count, 0);
          
          return html(`
            <div class="reactions-count">
              <i class="material-icons" style="color: #f59e0b; font-size: 1.2em;">favorite</i>
              <span style="margin-left: 4px; font-weight: 600;">${totalReactions}</span>
            </div>
          `);
        }
      },
...
/* Styles pour la colonne des réactions */
.reactions-count {
  display: flex;
  align-items: center;
  justify-content: center;
