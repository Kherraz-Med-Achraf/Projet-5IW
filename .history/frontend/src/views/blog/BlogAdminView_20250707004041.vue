...
const handlePostCreated = async () => {
  closeCreateModal();
  await reloadPosts();
  // Assurer que le grid est bien rafraîchi après une création
  await nextTick();
  if (!grid) {
    initGrid();
  }
...
const handlePostUpdated = async () => {
  closeEditModal();
  await reloadPosts();
  // Assurer que le grid est bien rafraîchi après une modification
  await nextTick();
  if (!grid) {
    initGrid();
  }
...
const confirmDelete = async () => {
  if (postToDelete.value) {
    ...
    await reloadPosts();
    // Assurer que le grid est bien rafraîchi après une suppression
    await nextTick();
    if (!grid) {
      initGrid();
    }
