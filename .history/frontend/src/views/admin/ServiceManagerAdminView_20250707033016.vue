...
async function fetchServiceManagers() {
  ...
  try {
    ...
    serviceManagers.value = [
      { id: 1, firstName: 'Pierre', lastName: 'Durand', email: 'pierre.durand@exemple.com', createdAt: '2024-01-15' },
      { id: 2, firstName: 'Sophie', lastName: 'Leroy', email: 'sophie.leroy@exemple.com', createdAt: '2024-02-20' },
    ];
    if (grid.value) {
      grid.value.updateConfig({ data: serviceManagers.value }).forceRender();
    }
  } catch (err: any) {
        ...
        formatter: (cell, row) => {
          ...
          return html(`
            <div class="action-buttons">
              <button class="btn-icon edit-btn" data-id="${serviceManager.id}" title="Modifier">
                <i class="material-icons">edit</i>
              </button>
              <button class="btn-icon delete-btn" data-id="${serviceManager.id}" title="Supprimer">
