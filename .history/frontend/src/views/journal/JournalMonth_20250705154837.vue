            ...
            <div v-for="att in attachments" :key="att.id" class="attachment-card">
              <div class="attachment-header">
                ...
                <div class="attachment-info">
                  ...
                  <small class="attachment-debug" v-if="!isSubmitted">
                    Debug: {{ JSON.stringify(att, null, 2) }}
                  </small>
                </div>
...
onMounted(async () => {
  try {
    ...
    if (existingJournal.value) {
      ...
      console.log('Attachments chargés:', attachments.value);
...
function viewAttachment(att: any) {
  ...
  // Basé sur l'interface Journal : { filepath: string }
  if (att.filepath) {
    // Si filepath commence par '/', c'est un chemin absolu depuis la racine du serveur
    if (att.filepath.startsWith('/')) {
      url = `http://localhost:3000${att.filepath}`;
    } else {
      url = `http://localhost:3000/${att.filepath}`;
    }
  } else if (att.id) {
    // Fallback : utiliser l'API d'attachments
