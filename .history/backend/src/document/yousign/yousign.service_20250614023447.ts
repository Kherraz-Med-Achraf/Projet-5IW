...
export class YousignService {
  ...
  private async uploadDocument(reqId: string, filePath: string) {
    ...
    fd.append('file', blob, filename);           // <input type="file" />
  ...
  private addSigner(reqId: string, name: { first: string; last: string }) {
    return this.call<SignerRes>(`signature_requests/${reqId}/signers`, {
      ...
      json: {
        ...
        /* duo accepté par l’API : */
        signature_level   : 'simple',      // « simple » = signature électronique de base
        authentication_mode: 'no_code',    // pas de SMS (sinon 'sms')
      },
    });
  }
  ...
  async createProcedure(consent: ConsentDocument, filePath: string) {
    ...
    await this.activateRequest(req.id);               // ouvre la signature
