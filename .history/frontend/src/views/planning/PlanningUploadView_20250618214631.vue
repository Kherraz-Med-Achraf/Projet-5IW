...
/** 3) Base events pour la semaine 1 **/
const baseWeekEvents = computed(() =>
  previewEntries.value
    .filter(e => e.staffId === selectedStaffId.value)
    .map((e, idx) => ({
      id: `${e.staffId}-${e.startTime}-${idx}`, // id unique
      title: e.activity,
      start: e.startTime,
      end: e.endTime,
...
/**
 * 5) Quand on change de semestre, on centre la vue sur sa date de début
 *    et on vide les events.
 */
...
/**
 * 6) Réplication de la même semaine sur tout le semestre
 */
watch(
  [ baseWeekEvents, selectedStaffId ],
  () => {
    ...
    const sem = planning.semesters.find(s => s.id === selectedSemesterId.value)
    if (!sem) return

    const start = new Date(sem.startDate)
    const end   = new Date(sem.endDate)
    const msPerWeek = 7 * 24 * 60 * 60 * 1000
    const weeksCount = Math.ceil((end.getTime() - start.getTime()) / msPerWeek)

    const all: typeof calendarOptions.value.events = []
    for (let w = 0; w < weeksCount; w++) {
      baseWeekEvents.value.forEach(ev => {
        const s = new Date(ev.start)
        const e = new Date(ev.end)
        s.setDate(s.getDate() + w * 7)
        e.setDate(e.getDate() + w * 7)
        all.push({ ...ev, start: s.toISOString(), end: e.toISOString(), id: `${ev.id}-w${w}` })
      })
    }

    calendarOptions.value.events = all
    console.log('Staff sélectionné :', selectedStaffId.value)
    console.log('Événements du semestre :', all)
  },
  { immediate: true }
)

/** 7) Réinitialiser au changement de semestre **/
