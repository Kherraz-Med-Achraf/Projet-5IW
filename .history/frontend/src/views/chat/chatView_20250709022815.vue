    ...
    <main class="profile-container" role="main" lang="fr" id="main-content">
      <div class="profile-content">
        <div class="content-grid">
          ...
          <PageHeader
            title="Messagerie"
            :subtitle="chats.length > 0 ? `${chats.length} conversation${chats.length > 1 ? 's' : ''}` : ''"
            icon="chat"
            :status-text="totalUnreadCount > 0 ? `${totalUnreadCount} message${totalUnreadCount > 1 ? 's' : ''} non lu${totalUnreadCount > 1 ? 's' : ''}` : ''"
            :status-icon="totalUnreadCount > 0 ? 'notifications' : ''"
            :status-class="totalUnreadCount > 0 ? 'status-warning' : ''"
          />
...
const openNewChatModal = async () => {
  try {
    // Les contacts sont déjà chargés dans init(), pas besoin de les recharger
    // sauf si la liste est vide (cas d'erreur)
...
/* =================== HEADER MODERNE (maintenant géré par PageHeader) =================== */
