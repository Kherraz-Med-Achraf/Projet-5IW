...
// Fonction pour générer le QR code depuis le otpSecret existant
async function generateQRCodeFromSecret(otpSecret: string) {
  // Construire l'URL otpauth avec le même label et issuer qu'au backend
...
// Au montage, si l'utilisateur a déjà un otpSecret, coche la case et génère le QR code
onMounted(() => {
...
// Observe auth.user pour mettre à jour l'état OTP
watch(
  () => auth.user,
  (newUser) => {
...
async function toggleOtp() {
  // Si la checkbox est cochée, activer l'OTP
  if (otpEnabled.value) {
    loading.value = true
    try {
      ...
      // Mise à jour du QR code et du secret retourné par le backend
    ...
    // Si la checkbox est décochée, désactiver l'OTP
    loading.value = true
    try {
