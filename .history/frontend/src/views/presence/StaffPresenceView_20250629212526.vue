...
function initGrid() {
  ...
  sheet.value.records.slice(0, 3).forEach((rec, index) => {
    ...
    console.log(`  - child.id: ${rec.child?.id}`)
  ...
  const data = sheet.value.records.map(rec => {
    const childId = rec.childId || rec.child?.id
    console.log(`Mapping record - childId: ${rec.childId}, child.id: ${rec.child?.id}, final: ${childId}`)
    return [
