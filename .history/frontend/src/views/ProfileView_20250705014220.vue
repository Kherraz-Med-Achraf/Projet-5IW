...
async function openEditModal() {
  try {
    ...
    const response = await SecureHttpClient.request(`${API_ENDPOINTS.AUTH.ENABLE_OTP.replace('/enable-otp', '/profile/edit')}`);
...
async function loadUserProfile() {
  try {
    const response = await SecureHttpClient.request('http://localhost:3000/auth/profile');
...
async function toggleOtp() {
  ...
  try {
    if (!otpEnabled.value) {
      ...
      const response = await SecureHttpClient.request('http://localhost:3000/auth/enable-otp', {
        method: 'POST'
      });
      ...
      const response = await SecureHttpClient.request('http://localhost:3000/auth/disable-otp', {
        method: 'POST'
      });
