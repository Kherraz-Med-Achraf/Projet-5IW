...
async function onSubmit() {
  auth.error = ''
  otpCode.value = '' // Réinitialise le code OTP
  // On envoie une connexion avec un otpCode vide
  const response = await auth.login({
    email: email.value,
    password: password.value,
    otpCode: '' // Initialement vide
  })

  if (response.access_token) {
    // Connexion réussie sans OTP requis
    toast.success('Vous êtes connecté !')
    router.push('/home')
  } else if (response.tempToken) {
    // Le backend demande une vérification OTP : on affiche la modale
...
async function submitOtp() {
  ...
  // On appelle verifyOtp avec le tempToken reçu précédemment
