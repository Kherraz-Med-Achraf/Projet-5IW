...
function getEventColor(activity: string): string {
  if (activity.toLowerCase().includes('annulé')) return '#B0BEC5'
...
const displayEvents = computed(() => {
  ...
  return events.value.map(event => {
    ...
    console.log('DEBUG EVENT:', {
      ...
      durationMinutes: (new Date(event.endTime) - new Date(event.startTime)) / (1000 * 60)
    })
...
.calendar-wrapper {
  ...
  // Style des événements - Forcer la hauteur correcte
  ...
  // Forcer la hauteur des conteneurs d'événements
