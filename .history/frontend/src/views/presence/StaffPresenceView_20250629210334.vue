  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cell: any, row: any) => {
        ...
        console.log('GridJS formatter - cell:', cell)
        console.log('GridJS formatter - row keys:', Object.keys(row))
        console.log('GridJS formatter - row.cells:', row.cells)
        if (row.cells && row.cells[2]) {
          console.log('GridJS formatter - row.cells[2]:', row.cells[2])
        }
        ...
        if (row && row.cells && Array.isArray(row.cells)) {
          ...
          console.log('Using row.cells format - childId:', childId, 'type:', typeof childId)
        } else if (Array.isArray(row)) {
          // Format direct avec array
          childId = row[2]
          isPresent = row[3]
          justificationType = row[4]
          console.log('Using direct array format - childId:', childId, 'type:', typeof childId)
        } else {
          console.error('Format de données GridJS non reconnu:', row)
          return html(`<span>Erreur de données</span>`)
        }
        
        console.log('Final extracted data:', { childId, isPresent, justificationType })
