...
export class ChildService {
  ...
  async removeForParent(parentProfileId: number, id: number) {
    // 1) Vérifier que l’enfant t’appartient et récupérer userId
    ...
    // 2) Transaction : supprimer d’abord le profil enfant, puis le compte User lié si présent
    await this.prisma.$transaction(async tx => {
      // supprime le profil enfant
      await tx.child.delete({ where: { id: child.id } });
      // supprime le compte enfant s’il existe
      if (child.userId) {
