...
export class YousignService {
  ...
  /* 2️⃣  upload PDF (⚠️ forcer une ext .pdf) ----------------------------- */
  private async uploadDocument(reqId: string, filePath: string) {
    ...
    /* si le nom tmp n’a pas d’extension on en ajoute une */
    const tmpName  = path.basename(filePath);
    const filename = tmpName.toLowerCase().endsWith('.pdf') ? tmpName : `${tmpName}.pdf`;
