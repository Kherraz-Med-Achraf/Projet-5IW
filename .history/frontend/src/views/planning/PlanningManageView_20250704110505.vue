...
const fcOptions: any = {
  ...
  eventContent(info: any) {
    ...
    return { html: `<div class="fc-event-content"><span class="fc-event-icon">${icon}</span> ${info.event.title}</div>` }
     ...
     const vacation = info.event.title.toLowerCase().includes('vacances') || 
                     info.event.title.toLowerCase().includes('férié')
     
     // Utiliser les dates string originales pour éviter les problèmes de timezone
     const originalEntry = events.value.find(e => e.id === info.event.id)
     
     selectedCourse.value = {
       id: info.event.id,
       title: info.event.title,
       start: originalEntry ? new Date(originalEntry.start) : info.event.start,
       end: originalEntry ? new Date(originalEntry.end) : info.event.end,
       children: kids,
       cancelled,
       vacation,
