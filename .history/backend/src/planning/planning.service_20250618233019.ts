import {
  ...
  export class PlanningService {
    ...
    /** Charge (et met en cache) les vacances Zone C pour toute une année */
    private async _loadVacationsForYear(year: number) {
      if (this.vacationsCache.has(year)) return;
      const url = `https://openholidaysapi.org/SchoolHolidays?countryIsoCode=FR&subdivisionCode=FR-C&languageIsoCode=FR&validFrom=${year}-01-01&validTo=${year}-12-31`;
      try {
        const res = await _fetch(url);
        if (!res.ok) throw new Error(`OpenHolidays error ${res.status}`);
        const data = (await res.json()) as any[];
        const periods = data.map(p => ({
          start: (p.startDate ?? p.start)?.substring(0, 10),
          end:   (p.endDate   ?? p.end)?.substring(0, 10),
        }));
        this.vacationsCache.set(year, periods);
      } catch (err) {
        console.error('Erreur chargement vacances', err);
        this.vacationsCache.set(year, []); // évite de retenter en boucle
      }
    }
