      ...
      if (typeof progressionData === 'object' && progressionData !== null) {
        Object.keys(progressionData).forEach(key => {
          ...
          if (typeof value === 'string') {
            form.progressionMissions[parseInt(key)] = value;
          } else if (typeof value === 'object' && value !== null) {
            // Si c'est un objet, on prend la propriété 'contenu' ou 'text' ou on stringify
            form.progressionMissions[parseInt(key)] = value.contenu || value.text || value.value || JSON.stringify(value);
          } else {
            form.progressionMissions[parseInt(key)] = String(value || '');
          }
        });
      }
