...
import {
  ...
  export class PresenceController {
    ...
    // 2. Validation de la feuille (éducateur)
    @Post(':id/validate')
    @Roles(Role.STAFF)
    validateSheet(
    ...
    // 4. Consultation par date (tous rôles)
    @Get()
  ...
  export class PresenceService {
    ...
    async validateSheet(
      ...
      const recordsData = children.map((child) => ({ sheetId, childId: child.id, present: presentSet.has(child.id) }));
      await this.prisma.presenceRecord.createMany({ data: recordsData, skipDuplicates: true });
  
      return this.prisma.presenceSheet.update({
        where: { id: sheetId },
        data: { staffId, validatedAtStaff: new Date(), status: 'PENDING_SECRETARY' },
      });
    }
  
    /**
     * Justification par la secrétaire
     */
    async justify(recordId: number, dto: JustifyAbsenceDto, filePath?: string) {
      ...
      await this.prisma.absenceJustification.create({ data: { recordId, justificationDate: new Date(dto.justificationDate), motif: dto.motif, filePath: filePath || null } });
  
      const pendingCount = await this.prisma.presenceRecord.count({ where: { sheetId: record.sheetId, present: false, justification: null } });
      const updateData: any = { validatedBySecretary: pendingCount === 0 };
      if (pendingCount === 0) { updateData.validatedAtSecretary = new Date(); updateData.status = 'VALIDATED'; }
  
      return this.prisma.presenceSheet.update({ where: { id: record.sheetId }, data: updateData });
    }
  
    /**
     * Récupère la feuille et ses enregistrements
     */
    async findByDate(dateString: string) {
      const date = new Date(dateString);
      const sheet = await this.prisma.presenceSheet.findUnique({ where: { date }, include: { records: { include: { child: true, justification: true } } } });
      if (!sheet) throw new NotFoundException(`Feuille non trouvée pour ${dateString}`);
      return sheet;
    }
  }
