...
function initGrid() {
  ...
  const data = sheet.value.records.map(record => {
    console.log('🔍 DEBUG Record:', {
      ...
      justificationType: record.justification?.type,
  ...
  console.log('🎯 DEBUG GridJS Data:', data)
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cell: any, row: any) => {
        ...
        // Récupération du childId et de la justification depuis les données de la ligne
        let childId = row.cells?.[2]?.data || row._data?.[2] || row.data?.[2]
        let justification = row.cells?.[3]?.data || row._data?.[3] || row.data?.[3]
        ...
        console.log('🔍 DEBUG Formatter:', {
          ...
          isValidated,
          rowCells: row.cells?.map((c: any) => c?.data),
          rowData: row._data || row.data
        })
