...
function initGrid() {
  ...
  // Debug: Inspecter la structure des records
  console.log('=== RECORDS STRUCTURE DEBUG ===')
  sheet.value.records.slice(0, 3).forEach((rec, index) => {
    console.log(`Record ${index}:`, rec)
    console.log(`  - id: ${rec.id}`)
    console.log(`  - childId: ${rec.childId}`)
    console.log(`  - present: ${rec.present}`)
    console.log(`  - child:`, rec.child)
    console.log('  - Keys in rec:', Object.keys(rec))
    console.log('  - Keys in rec.child:', Object.keys(rec.child || {}))
  })
  ...
  const data = sheet.value.records.map(rec => {
    console.log(`Mapping record - childId: ${rec.childId}, final: ${rec.childId}`)
    return [
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cell: any, row: any) => {
        ...
        // Debug ULTRA détaillé pour comprendre la structure
        console.log('=== FORMATTER ULTRA DEBUG ===')
        console.log('cell:', cell)
        console.log('row:', row)
        console.log('row.cells:', row.cells)
        console.log('row.cells length:', row.cells?.length)
        
        // Debug chaque cellule individuellement
        if (row.cells) {
          row.cells.forEach((cellItem: any, index: number) => {
            console.log(`row.cells[${index}]:`, cellItem)
            if (cellItem && typeof cellItem === 'object') {
              console.log(`row.cells[${index}].data:`, cellItem.data)
              console.log(`row.cells[${index}] keys:`, Object.keys(cellItem))
            }
          })
        }
        ...
        if (row.cells && row.cells[2]) {
          console.log('Tentative 1 - row.cells[2].data:', row.cells[2].data)
          console.log('Tentative 2 - row.cells[2]:', row.cells[2])
          
          // Essayer d'accéder directement si c'est un objet
          if (typeof row.cells[2] === 'object' && row.cells[2].data !== undefined) {
            childId = row.cells[2].data
          } else if (typeof row.cells[2] === 'string' || typeof row.cells[2] === 'number') {
            childId = row.cells[2]
          }
        }
        
        // Si toujours pas trouvé, essayer avec la propriété _data
        if (!childId && row._data && Array.isArray(row._data)) {
          console.log('Tentative 3 - row._data:', row._data)
          childId = row._data[2]
        }
        
        // Si toujours pas trouvé, essayer avec row.data
        if (!childId && row.data && Array.isArray(row.data)) {
          console.log('Tentative 4 - row.data:', row.data)
          childId = row.data[2]
        }
        
        console.log('Final childId après toutes tentatives:', childId, 'type:', typeof childId)
        
        if (childId === undefined || childId === null) {
          console.error('childId STILL missing after all attempts, row structure:', row)
          return html(`<span style="color: red;">ID manquant (debug: cells=${row.cells?.length})</span>`)
        }
...
/* Table container avec meilleure présentation */
.table-container {
  margin-top: 1.5rem;
  border-radius: 1rem;
  overflow: hidden;
...
/* Amélioration du wrapper du grid */
:deep(.grid-wrapper) {
  border-radius: 1rem;
  overflow: hidden;
}
...
/* Grid headers avec police Satoshi */
:deep(.grid-header) {
...
/* Grid cells avec police Satoshi */
:deep(.grid-cell) {
  padding: 0.75rem;
...
/* Badge styles - Meilleur alignement et couleurs */
.badge {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
