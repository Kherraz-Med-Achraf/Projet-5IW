...
export class PresenceController {
  ...
  /**
   * 5. Endpoint de test pour déclencher manuellement le cron de création des feuilles
  ...
  /**
   * 6. Endpoint temporaire pour créer une feuille aujourd'hui (contournement production)
   * TEMPORAIRE - à supprimer après résolution du problème
   */
  @Post('create-today')
  @Roles(Role.STAFF, Role.DIRECTOR, Role.SERVICE_MANAGER)
  @ApiOperation({ summary: 'Créer la feuille pour aujourd\'hui (contournement temporaire)' })
  async createTodaySheet(@Req() req: Request & { user: { id: string; role: Role } }) {
    try {
      ...
      return {
        success: true,
        message: `Feuille créée pour ${today}`,
        sheet: this._mapParentPhones(sheet),
        timestamp: new Date().toISOString(),
      };
    } catch (error: any) {
      return {
        success: false,
        message: 'Erreur lors de la création de la feuille',
        error: error.message,
        timestamp: new Date().toISOString(),
      };
    }
