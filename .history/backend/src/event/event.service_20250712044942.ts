...
export class EventService {
  ...
  async listUpcoming() {
    ...
    const withCap = await Promise.all(
      events.map(async (ev) => {
        if (ev.capacity && ev.capacity < 100) {
  ...
  async create(dto: CreateEventDto, creatorUserId: string, imageUrl?: string) {
    ...
    return this.prisma.event.create({
      data: {
        ...
        capacity: dto.capacity ?? 100, // 100 places par défaut si non spécifié (affiché comme "illimité")
  ...
  async update(
    ...
    return this.prisma.event.update({
      ...
      data: {
        ...
        ...(dto.capacity !== undefined && { capacity: dto.capacity || 100 }),
