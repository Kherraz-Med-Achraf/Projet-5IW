...
// On initialise le checkbox OTP en fonction de la présence du otpSecret
onMounted(() => {
  otpEnabled.value = !!(auth.user && auth.user.otpSecret)
})

// On observe le user pour réagir si l'otpSecret change
watch(
  () => auth.user,
  (newUser) => {
    otpEnabled.value = !!(newUser && newUser.otpSecret)
  },
  { immediate: true }
)
...
async function toggleOtp() {
  ...
  if (otpEnabled.value) {
    ...
    try {
      ...
      // On appelle la route POST /auth/enable-otp
      ...
      // On suppose que le backend renvoie { message, secret, qrCodeDataUrl } :
      qrCodeDataUrl.value = data.qrCodeDataUrl || ''
      secret.value = data.secret || ''
      // On met à jour auth.user.otpSecret pour refléter l'activation
      if (auth.user) {
      ...
      // Si ça échoue, on remet la checkbox à false
    ...
    try {
      ...
      toast.error(errorMsg)
      // Si ça échoue, on remet la checkbox à true
