...
const calendarEvents = computed(() => {
  const raw = previewEntries.value
    ...
    .map(e => ({
      ...
      color: e.activity.startsWith('Annulé –') ? '#B0BEC5' : (e.activity.toLowerCase().includes('vacances') ? '#FFB74D' : undefined),
...
const calendarOptions = ref({
  ...
  eventClick(info: any) {
    ...
    if (kids.length) {
      ...
      alert('Aucun enfant associé à cette activité')
    }
  },
  events: [] as any[],
})
