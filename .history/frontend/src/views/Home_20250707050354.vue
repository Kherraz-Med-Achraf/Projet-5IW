...
const todayPresenceSheet = computed(() => {
  return presenceStore.currentSheet;
});

const presentToday = computed(() => {
  return todayPresenceSheet.value?.records?.filter(r => r.status === 'present')?.length || 0;
});

const absentToday = computed(() => {
  return todayPresenceSheet.value?.records?.filter(r => r.status === 'absent')?.length || 0;
});

const lateToday = computed(() => {
  return todayPresenceSheet.value?.records?.filter(r => r.status === 'late')?.length || 0;
});
