...
async function onReopen() {
  ...
  try {
    if (existingJournal.value?.isSubmitted) {
      const reopened = await journalStore.reopenJournal(
...
function exportReport() {
  ...
  /* --- evolution des missions (si progression) ------------------------- */
  const hasProgression = missions.value.some(m => {
    const val = form.progressionMissions[m.id]
  ...
  if (hasProgression) {
    ...
    missions.value.forEach(m => {
      ...
      if (prog && prog.trim().length) {
        const wrapped = doc.splitTextToSize(`• ${m.description} → ${prog.trim()}`, 170)
