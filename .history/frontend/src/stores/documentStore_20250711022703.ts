...
export const useDocumentStore = defineStore('document', {
  ...
  actions: {
    ...
    /** Obtenir le lien de signature YouSign **/
    async getYouSignSignatureLink(documentId: string) {
      this.loading = true;
      this.error = null;
      const notify = useNotificationStore();

      try {
        const response = await secureApiCall(`${API}/documents/${documentId}/signature-link`);
        ...
        console.log(`🔗 Lien de signature récupéré pour le document ${documentId}`);
        return data.signatureLink;
        
      } catch (err: any) {
        this.error = err.message;
        notify.showNotification(this.error || 'Erreur lors de la récupération du lien de signature', 'error');
        console.error('❌ Erreur récupération lien signature:', err);
        throw err;
      } finally {
        this.loading = false;
      }
    ...
    /** Rediriger vers la signature YouSign **/
    async redirectToYouSignSignature(documentId: string) {
      const notify = useNotificationStore();

      try {
        const signatureLink = await this.getYouSignSignatureLink(documentId);
        
        // Ouvrir YouSign dans une nouvelle fenêtre
        ...
        if (!signatureWindow) {
          throw new Error('Impossible d\'ouvrir la fenêtre de signature. Veuillez vérifier que les pop-ups sont autorisés.');
        }

        // Optionnel : écouter la fermeture de la fenêtre pour actualiser les données
        const checkClosed = setInterval(() => {
          if (signatureWindow.closed) {
            clearInterval(checkClosed);
            console.log('🔄 Fenêtre de signature fermée, actualisation des données...');
            this.fetchDocuments(); // Actualiser la liste des documents
          }
        }, 1000);

        notify.showNotification('Redirection vers la signature électronique...', 'info');
        
      } catch (err: any) {
        notify.showNotification(err.message || 'Erreur lors de la redirection vers la signature', 'error');
        console.error('❌ Erreur redirection signature:', err);
        throw err;
      }
    },
    ...
    async markDocumentAsViewed(documentId: string) {
      try {
        const response = await secureApiCall(`${API}/documents/${documentId}/mark-viewed`, {
          method: 'POST',
        });

        if (!response.ok) {
          console.warn('Échec marquage document comme consulté');
          return;
        }
        ...
        const doc = this.documents.find(d => d.id === documentId);
        if (doc && doc.access) {
          doc.access.viewedAt = new Date().toISOString();
        }

        console.log(`👁️ Document marqué comme consulté: ${documentId}`);
        
      } catch (err: any) {
        console.warn('Erreur marquage document consulté:', err);
      }
    },
    ...
    async getDocumentDetails(documentId: string) {
      const response = await secureJsonCall(`${API}/documents/${documentId}`);
      return response;
    },
     ...
     */
    async addDocumentAccess(documentId: string, parentIds: number[]) {
      const response = await secureJsonCall(`${API}/documents/${documentId}/access`, {
        method: 'POST',
        data: { parentIds },
      });
      return response;
    ...
    /**
     * Retirer l'accès à un document pour des parents
     */
    async removeDocumentAccess(documentId: string, parentIds: number[]) {
      const response = await secureJsonCall(`${API}/documents/${documentId}/access`, {
        method: 'DELETE',
        data: { parentIds },
      });
    ...
    /**
     * Récupérer la liste des parents pour les documents (SECRETARY)
     */
    async getParentsForDocuments() {
      const response = await secureJsonCall(`${API}/documents/parents`)
      return response
    },
  },
}); 