...
export class EventService {
  ...
  async confirmStripePayment(sessionId: string) {
    ...
    try {
      console.log(`📧 Attempting to send confirmation email for registration ${reg.id}`);
      await this._sendRegistrationMail(reg.id);
      console.log(`📧 Confirmation email sent for registration ${reg.id}`);
    } catch (error) {
      console.error(`❌ Failed to send confirmation email for registration ${reg.id}:`, error);
