...
export const useDocumentStore = defineStore('document', {
  ...
  actions: {
    async fetchSecretary(page = 1) {
      ...
      try {
        const res = await fetch(
          `/documents/consents/secretary?page=${page}`,
        );
    ...
    async fetchParent(page = 1) {
      ...
      try {
        const res = await fetch(
          `/documents/consents/parent?page=${page}`,
        );
        if (!res.ok) throw new Error(await res.text());
    ...
    async uploadConsent(formData: FormData) {
      const res = await fetch(`/documents/consents`, {
        method: 'POST',
        body: formData,
      });
      if (!res.ok) throw new Error(await res.text());
    ...
    async getSignUrl(id: string) {
      const res = await fetch(`/documents/consents/${id}/sign-url`);
      if (!res.ok) throw new Error(await res.text());
