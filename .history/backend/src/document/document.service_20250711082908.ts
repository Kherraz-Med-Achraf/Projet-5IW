...
export class DocumentService {
  ...
  async downloadDocument(documentId: string, userId: string, userRole: Role) {
    ...
    try {
      ...
      console.error(`Métafichier: ${metaFilepath}`);
      
      // Vérifier si les fichiers existent
      try {
        await fs.access(filepath);
        console.log(`✓ Fichier principal existe: ${filepath}`);
      } catch (e) {
        console.error(`✗ Fichier principal manquant: ${filepath}`);
      }
      
      try {
        await fs.access(metaFilepath);
        console.log(`✓ Métafichier existe: ${metaFilepath}`);
      } catch (e) {
        console.error(`✗ Métafichier manquant: ${metaFilepath}`);
      }
