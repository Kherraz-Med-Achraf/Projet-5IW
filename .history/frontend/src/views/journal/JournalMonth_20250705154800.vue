...
function getAttachmentName(att: any): string {
  return att.originalName || att.fileName || att.name || att.filename || 'Fichier sans nom';
}

function getAttachmentType(att: any): string {
  return att.type || att.mimeType || att.mimetype || att.fileType || 'Type inconnu';
}

function viewAttachment(att: any) {
  console.log('Viewing attachment:', att);
  
  // Essayer différentes propriétés pour l'URL
  let url = att.url || att.filePath || att.path || att.src || att.downloadUrl;
  
  // Si on a un nom de fichier, essayer de construire l'URL depuis le backend
  if (!url && att.fileName) {
    url = `/api/journal/attachment/${att.id}`;
  }
  ...
  if (url) {
    ...
    toast.error('Impossible d\'ouvrir le fichier : URL manquante');
  }
}
