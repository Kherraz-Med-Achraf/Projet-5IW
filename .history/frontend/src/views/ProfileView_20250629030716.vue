      ...
      <div class="content-grid">
        ...
        <div v-if="auth.user?.role === 'PARENT'" class="profile-section children-section">
          <div class="section-header">
            ...
            <button class="edit-btn" @click="openChildModal()">
            ...
            <div v-for="child in children" :key="child.id" class="child-card">
              <div class="child-info">
                <div class="child-header">
                  ...
                  <button class="edit-child-btn" @click="openChildModal(child)">
                    <i class="material-icons">edit</i>
                  </button>
                </div>
    ...
    <div v-if="showChildModal" class="modal-overlay" @click="closeChildModal">
      <div class="modal-content child-modal" @click.stop>
        <div class="modal-header">
          <h3>{{ editingChild ? 'Modifier l\'enfant' : 'Ajouter un enfant' }}</h3>
...
async function loadUserProfile() {
  try {
    ...
    if (response.ok) {
      ...
      console.log('User role:', userProfile.value.role);
      console.log('First name:', userProfile.value.firstName);
      console.log('Last name:', userProfile.value.lastName);
      console.log('Children:', userProfile.value.children);
    } else {
...
async function saveChild() {
  ...
  try {
    const url = editingChild.value 
      ? `http://localhost:3000/children/${editingChild.value.id}`
      : 'http://localhost:3000/children';
    
    const method = editingChild.value ? 'PATCH' : 'POST';
    ...
    if (response.ok) {
      ...
      if (editingChild.value) {
        ...
        children.value.push(savedChild);
      ...
      const errorText = await response.text();
      console.error('Failed to save child:', response.status, errorText);
...
async function deleteChild() {
  if (!editingChild.value || childLoading.value) return;
  
  if (!confirm(`Êtes-vous sûr de vouloir supprimer ${editingChild.value.firstName} ${editingChild.value.lastName} ?`)) {
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      children.value = children.value.filter(c => c.id !== editingChild.value.id);
      ...
      const errorText = await response.text();
      console.error('Failed to delete child:', response.status, errorText);
