<template>
    <dialog ref="dialog" class="p-6 bg-white rounded-xl shadow-xl w-96">
      <form @submit.prevent="submit" class="space-y-4">
        ...
        <div class="flex justify-end gap-2 pt-4">
          ...
          <button type="submit" class="btn btn-primary" :disabled="!selectedId">Envoyxcvxcver</button>
        </div>
      </form>
    </dialog>
  </template>
  ...
  import { useAuthStore } from '@/stores/auth'      // 👉 pour récupérer le token
  ...
  const headers = auth.token
    ...
    : {}                                             // token lu dans le store
  
  /* charge toute la liste à l’ouverture */
  onMounted(async () => {
