...
export class AuthService {
  ...
  async refreshToken(token: string) {
    try {
      ...
      // Si pas d'utilisateur ou pas de refreshToken stocké → invalide
      if (!user || !user.refreshToken) {
        throw new HttpException('Refresh token invalide', HttpStatus.UNAUTHORIZED);
      }
  
      // Vérifie le token reçu contre le hash en BDD
      const isMatch = await bcrypt.compare(token, user.refreshToken);
      if (!isMatch) {
        // Révoque le refreshToken en base si l’ancien est réutilisé / compromis
        await this.prisma.user.update({
          where: { id: user.id },
      ...
      // Génère et stocke le nouveau refreshToken
      const tokens = this.generateTokens(user.id, user.email, user.role);
      await this.storeHashedRefreshToken(user.id, tokens.refresh_token);
      return tokens;
    } catch {
  ...
  async enableOtp(userId: string) {
    ...
    const secret = speakeasy.generateSecret({ length: 20, name: 'MyApp (ex: Google Auth Label)', issuer: 'MyApp Issuer' });
...
async disableOtp(userId: string) {
  // 1. on met otpSecret à null
  const user = await this.prisma.user.update({
    where: { id: userId },
    data: { otpSecret: null },
  })

  // 2. on génère une nouvelle paire de tokens
  ...
  // 3. on stocke le hash du nouveau refresh_token en BDD
  await this.storeHashedRefreshToken(user.id, refresh_token)

  // 4. on retourne tout ce qu’attend le contrôleur
  return {
    access_token,
    refresh_token,
