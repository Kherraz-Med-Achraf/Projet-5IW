    ...
    },
    {
      ...
      formatter: (cell: any, row: any) => {
        ...
        if (row.cells && row.cells[2]) {
          console.log('row.cells[2]:', row.cells[2])
          console.log('row.cells[2].data:', row.cells[2].data)
        }
        
        const childId = row.cells[2]?.data  // colonne childId (index 2)
        
        console.log('Final childId:', childId, 'type:', typeof childId)
        
        if (childId === undefined || childId === null) {
          console.error('childId is missing, cannot create checkbox')
