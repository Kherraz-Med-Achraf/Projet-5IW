...
export class ChildService {
  ...
  async removeForParent(parentProfileId: number, id: number) {
    // 1) Vérifier que cet enfant t’appartient vraiment
    ...
    // 2) Supprimer le compte utilisateur lié (s’il existe)
    await this.prisma.user.deleteMany({
      where: { childProfile: { id } },  // ← ici on filtre sur la relation
    });
  
    // 3) Supprimer enfin le profil enfant
