...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(@ConnectedSocket() socket: Socket) {
    ...
    try {
      ...
      // Différencier les erreurs JWT normales (token expiré) des erreurs suspectes
      if (error.message === 'jwt expired' || error.message === 'invalid token') {
        console.log(`[AUTH] Token expiré/invalide pour connexion depuis ${clientIp}`);
        socket.emit('auth_error', { message: 'Session expirée, veuillez vous reconnecter' });
        // Ne pas marquer comme activité suspecte pour un token simplement expiré
      } else {
        // Autres erreurs JWT peuvent être suspectes (token malformé, etc.)
