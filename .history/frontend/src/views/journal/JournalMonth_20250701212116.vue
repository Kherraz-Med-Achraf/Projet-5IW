          ...
          <div class="observation-content">
            <div class="observation-input-container">
              ...
              <textarea
                ...
                :aria-describedby="isSubmitted ? 'obs-readonly-help' : 'obs-input-help'"
              ></textarea>
              ...
              <div v-else id="obs-input-help" class="help-text">
                Décrivez factuellement les observations réalisées ce mois-ci
              </div>
            </div>
              ...
              <div class="mission-card-content">
                <div class="progress-input-container">
                  ...
                  <textarea
                    ...
                    :aria-describedby="isSubmitted ? `mission-${m.id}-readonly` : `mission-${m.id}-help`"
                  ></textarea>
                  ...
                  <div v-else :id="`mission-${m.id}-help`" class="help-text">
                    Décrivez les progrès et difficultés observés pour cette mission
                  </div>
                </div>
        ...
        <section class="profile-section attachments-section" id="attachments-section" aria-labelledby="attachments-title">
          ...
          <div v-if="!isSubmitted && attachments.length < 3" class="upload-section-clean" role="region" aria-labelledby="upload-title">
            ...
            <div class="upload-input-container">
              ...
              />
              <div v-if="selectedFile" id="file-selected" class="help-text success">
                <span class="material-icons" aria-hidden="true">check_circle</span>
                Fichier sélectionné : {{ selectedFile.name }}
              </div>
              <div v-else id="file-help" class="help-text info">
                <span class="material-icons" aria-hidden="true">info</span>
                Formats acceptés : PDF, DOC, DOCX, JPG, PNG (max 10Mo)
              </div>
              <button
...
function sanitizeDisplayName(filename: string): string {
  // Fonction de translittération robuste
  const transliterate = (str: string): string => {
    ...
    return str.toLowerCase().replace(/[àáâãäåæçčćèéêëěęìíîïñňńòóôõöøœùúûüůÿýßđřšśťžźż]/g, 
  ...
  return transliterate(filename)
    ...
    .replace(/[^a-zA-Z0-9.-]/g, '_') // Remplace tous les autres caractères spéciaux par des underscores
...
function onFileChange(e: Event) {
  ...
  if (input.files?.length) {
    ...
    const sanitizedName = sanitizeDisplayName(originalName);
    
    if (!sanitizedName || sanitizedName.length < 1) {
      toast.error('Le nom de fichier contient uniquement des caractères spéciaux. Veuillez renommer votre fichier avec au moins quelques lettres ou chiffres.');
      input.value = '';
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error('Format de fichier non autorisé. Veuillez sélectionner un fichier PDF, DOC, DOCX, JPG ou PNG.');
      input.value = '';
      selectedFile.value = null;
      return;
    }
    ...
    if (file.size > maxSize) {
      toast.error('Le fichier est trop volumineux. Taille maximum autorisée : 10Mo.');
      input.value = '';
      selectedFile.value = null;
      return;
    }
    
    // Avertir l'utilisateur si le nom va être modifié
    if (originalName !== sanitizedName) {
      toast.info(`Le nom de fichier sera nettoyé : "${sanitizedName}"`);
    }
...
async function onUpload() {
  ...
  try {
    ...
    const displayName = att.filepath;
    selectedFile.value = null;
    ...
    toast.success(`Fichier "${displayName}" ajouté avec succès`);
  } catch (e: any) {
