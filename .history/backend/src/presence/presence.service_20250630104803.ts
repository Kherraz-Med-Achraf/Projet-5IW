...
export class PresenceService {
  private readonly encryptionKey = process.env.FILE_ENCRYPTION_KEY!; // Non-null après validation
  ...
  private validateEncryptionKey(): void {
    ...
    if (isProduction) {
      if (!encryptionKey) {
        throw new Error('❌ PRODUCTION: FILE_ENCRYPTION_KEY manquante. Application arrêtée pour sécurité.');
      }
      
      if (encryptionKey === 'default-key-change-me') {
        throw new Error('❌ PRODUCTION: Clé de chiffrement par défaut détectée. Changez FILE_ENCRYPTION_KEY.');
      }
      
      if (encryptionKey.length < 32) {
        throw new Error('❌ PRODUCTION: FILE_ENCRYPTION_KEY trop courte (minimum 32 caractères).');
      }
      
      this.logSecure('info', 'Clé de chiffrement validée pour la production', {});
      return;
    }

    // En développement : validation plus souple avec avertissements
    if (!encryptionKey) {
      this.logSecure('warn', '⚠️ DÉVELOPPEMENT: FILE_ENCRYPTION_KEY manquante. Utilisation clé temporaire.', {
        recommendation: 'Ajoutez FILE_ENCRYPTION_KEY=your-32-character-key-here à votre .env'
      });
      ...
      this.logSecure('info', 'Clé de chiffrement temporaire générée pour le développement', {
    ...
    if (encryptionKey === 'default-key-change-me') {
      this.logSecure('warn', '⚠️ DÉVELOPPEMENT: Clé par défaut détectée. Recommandé de la changer.', {});
    }
    
    if (encryptionKey.length < 32) {
      this.logSecure('warn', '⚠️ DÉVELOPPEMENT: Clé trop courte. Recommandé minimum 32 caractères.', {
        currentLength: encryptionKey.length
      });
    }

    this.logSecure('info', 'Validation clé de chiffrement terminée en mode développement', {
      environment: nodeEnv,
  ...
  private async validateFileContent(filePath: string, expectedMimeType: string): Promise<boolean> {
    try {
      ...
      if (fileBuffer.length < 12) {
        this.logSecure('warn', 'Fichier trop petit pour être valide', { expectedMimeType });
        return false;
      }
      ...
      switch (expectedMimeType) {
        ...
        default:
          this.logSecure('warn', 'Type MIME non supporté pour validation', { expectedMimeType });
          return false;
      }
    } catch (error) {
      this.logSecure('error', 'Erreur validation fichier', { error: error.message, expectedMimeType });
      return false;
    }
  }
