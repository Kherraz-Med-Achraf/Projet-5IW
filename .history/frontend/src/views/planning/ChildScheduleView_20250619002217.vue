...
async function loadChildren() {
  ...
  const res = await fetch(`${API}/parent/my-children`, { // suppose endpoint
...
async function loadSchedule() {
  ...
  events.value = data.map((e:any)=>({ id:e.id,title:e.activity,start:e.startTime,end:e.endTime,allDay:e.activity.toLowerCase().includes('vacances') }))
...
<style scoped>
.child-schedule { max-width: 800px; margin:auto; }
