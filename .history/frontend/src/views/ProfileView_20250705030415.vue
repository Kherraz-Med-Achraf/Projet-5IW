...
async function loadUserProfile() {
  try {
    ...
    console.log('[ProfileView] Auth store token:', token ? `Present (${token.substring(0, 20)}...)` : 'Missing');
    console.log('[ProfileView] Legacy token:', legacyToken ? `Present (${legacyToken.substring(0, 20)}...)` : 'Missing');
    console.log('[ProfileView] Final token used:', finalToken ? `Present (${finalToken.substring(0, 20)}...)` : 'Missing');
    console.log('[ProfileView] Auth user:', auth.user);
    console.log('[ProfileView] Auth isAuthenticated:', auth.isAuthenticated);
    ...
    if (response.ok) {
      ...
      if (response.status === 401) {
        console.log('[ProfileView] 401 Unauthorized - clearing auth and redirecting to login');
...
async function toggleOtp() {
  ...
  try {
    ...
    console.log('[OTP] Using token:', finalToken ? 'Present' : 'Missing');
...
async function saveContact() {
  ...
  try {
    // Récupérer l'ID du profil parent depuis les enfants ou contacts d'urgence
    let parentProfileId = null;
    
    // Essayer de récupérer depuis les enfants
    if (userProfile.value?.children && userProfile.value.children.length > 0) {
      parentProfileId = userProfile.value.children[0].parentProfileId;
    }
    // Sinon, essayer de récupérer depuis les contacts d'urgence (si on peut ajouter cette info)
    else if (userProfile.value?.emergencyContacts && userProfile.value.emergencyContacts.length > 0) {
      // Pour l'instant, on va utiliser une approche différente
      throw new Error('Impossible de déterminer l\'ID du profil parent');
    }
    
    if (!parentProfileId) {
      throw new Error('Profil parent non trouvé - aucun enfant associé');
    }
