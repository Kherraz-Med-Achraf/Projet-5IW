import {
  ...
  export class PlanningService {
    ...
    private async _expandEntriesOverSemester(
        ...
        for (let w = 0; w < totalWeeks; w++) {
          ...
          for (const be of baseEntries) {
            ...
            // Détermine si c'est un jour férié ou des vacances -> label spécial à afficher
            const closureLabel = this._getClosureLabel(entryDate);
      
            if (closureLabel) {
              // On évite les doublons : si un placeholder existe déjà pour ce jour + staff, on passe
              ...
              if (exists) continue;
              // On ajoute un créneau placeholder pour indiquer la fermeture
              result.push({
                ...be,
              ...
              continue; // on ne duplique pas la séance normale ce jour-là
            }
    ...
    private _getClosureLabel(date: Date): string | null {
      // Jour férié national
      if (hd.isHoliday(this._atNoon(date))) return 'Jour férié';
