...
describe('AuthService – login() sans OTP', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(bcrypt, 'hash').mockResolvedValue('REFRESH_HASH' as any)
    ;(jwtMock.sign as jest.Mock).mockReturnValue('jwt-token')
  })
...
describe('AuthService – login() avec OTP', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(bcrypt, 'compare').mockResolvedValue(true as any)
    jest.spyOn(require('speakeasy'), 'totp').mockReturnValue({ verify: jest.fn(() => true) } as any)
    jest.spyOn(require('speakeasy'), 'verify').mockImplementation()
    jest.spyOn(require('speakeasy'), 'totp')
    jest.spyOn(require('speakeasy'), 'verify')

    jest.spyOn(require('speakeasy'), 'totp').mockRestore()

    jest.spyOn(require('speakeasy'), 'totp').mockImplementation()

    jest.spyOn(require('speakeasy'), 'totp').mockClear()
  })
}) 