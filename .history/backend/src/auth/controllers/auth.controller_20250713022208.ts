...
export class AuthController {
  ...
  async initiateLogin(
    ...
    const result = await this.authService.initiateLogin(body.email, body.password);
    
    // Générer un token CSRF même pour les logins avec mot de passe expiré
    const prod = process.env.NODE_ENV === 'production';
    const csrf = randomToken(16);
    res.cookie('csrf_token', csrf, {
      httpOnly: false,
