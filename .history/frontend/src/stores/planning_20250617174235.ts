...
export const usePlanningStore = defineStore('planning', {
  state: () => ({
    ...
    /** Liste des userId déjà importés */
    importedStaffIds: [] as string[],

    /** Tous les créneaux agrégés pour le semestre sélectionné */
  ...
  actions: {
    ...
    selectSemester(id: string) {
      ...
      // remise à zéro des imports/aperçu si on change de semestre
      this.importedStaffIds = []
      this.allEntries = []
    },

    /**
     * 1. Importer l’Excel pour un éducateur (persist partiel)
    ...
    /**
     * 2. Prévisualiser l’ensemble des imports déjà effectués
     * @param semesterId ID du semestre
     */
    async fetchAggregated(semesterId: string) {
      this.loading = true
      this.error = null
      this.allEntries = []
      const auth = useAuthStore()
      try {
        const res = await fetch(
          `${API}/planning/semesters/${semesterId}/overview`,
          {
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        )
    ...
    /**
     * 3. Valider (soumettre) le planning final pour le semestre
     * @param semesterId ID du semestre
     */
    async submitPlanning(semesterId: string) {
      this.loading = true
      this.error = null
      const auth = useAuthStore()
      try {
        const res = await fetch(
          `${API}/planning/semesters/${semesterId}/submit`,
          {
            method: 'POST',
            headers: { Authorization: `Bearer ${auth.token}` },
          }
        ...
        if (!res.ok) throw new Error(await res.text())
        // on peut éventuellement réinitialiser le store ici
      } catch (err: any) {
        this.error = err.message
      } finally {
        this.loading = false
