...
private _isVacationWeek(date: Date): boolean {
  // 1) si c'est un jour férié
  if (hd.isHoliday(date)) return true

  // 2) si la date appartient à une période de vacances zone C
  const vacations = hd.getVacations(2025 /* ou l'année courante */)
    .filter(v => v.substring.includes('Zone C'))
  return vacations.some(vac => date >= new Date(vac.start) && date <= new Date(vac.end))
}
