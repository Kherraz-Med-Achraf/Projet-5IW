...
/* ——————————————————— types de réponses (très simplifiés) ——————————————————— */
...
export class YousignService {
  ...
  /* 1️⃣  Signature-request (livraison e-mail) */
  ...
  /* 2️⃣  Upload PDF (multipart) — on force l’extension .pdf */
  ...
  /* 3️⃣  Ajout d’un signer : **PAS** d’`authentication_mode` ni `redirect_url` */
  private addSigner(
    reqId: string,
    ...
    return this.call<SignerRes>(
      `signature_requests/${reqId}/signers`,
      {
        ...
        json  : {
          ...
          /* niveau “électronique” (simple) par défaut ; aucun autre champ */
  ...
  /* 4️⃣  Activation instantanée de la demande */
  private activateRequest(reqId: string) {
    return this.call(`signature_requests/${reqId}/activate`, { method: 'POST' });
  }

  /* ██████████████  API exposée au DocumentService  ██████████████ */

  /**  Lance toute la procédure et renvoie le lien de signature secrétaire */
  async createProcedure(
    ...
    /* 3. Signers (secrétaire PUIS parent) */
