<template>
  <div class="yearly-student-import">
    ...
    <!-- Bandeau d'import (année auto N‑N+1) -->
    <div class="button-bar">
      ...
      <input ref="fileInput" type="file" accept=".xlsx, .xls" @change="handleFile" class="hidden" />
    </div>
    ...
    <div v-else-if="parsedData.length" class="preview">
      ...
      <div class="search-bar">
        <input v-model="searchTerm" @input="currentPage = 1" type="text" placeholder="Rechercher par nom ou prénom" class="search-input" />
      </div>
      ...
      <table class="preview-table">
        ...
        <tbody>
          <tr v-for="(row, index) in paginatedData" :key="index">
            ...
            <td><button @click="removeRow((currentPage-1)*pageSize + index)" class="delete-button">Supprimer</button></td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showConfirm" class="modal-overlay">
      <div class="modal">
        ...
        <p>Cette opération enregistrera la liste {{ schoolYearLabel }} de façon définitive. Continuer&nbsp;?</p>
        <div class="modal-actions">
          <button @click="sendToBackend" class="confirm-button" :disabled="isSubmitting">Oui, valider</button>
          <button @click="showConfirm = false" class="delete-button" :disabled="isSubmitting">Annuler</button>
        </div>
      </div>
    </div>
  </div>
...
if (!auth.isAuthenticated || auth.user?.role !== 'SECRETARY') router.replace({ name: 'Home' })
...
async function handleFile(e:Event){
  ...
  const {read,utils}=await import('xlsx'); const wb=read(await file.arrayBuffer(),{type:'array'});
  const sheet=wb.Sheets[wb.SheetNames[0]]; const json:any[]=utils.sheet_to_json(sheet)
  if(!json.length||!('NOM'in json[0])||!('Prénom'in json[0])){errorMessage.value='Le fichier doit contenir les colonnes "NOM" et "Prénom".';parsedData.value=[];return}
  ...
  parsedData.value=valid;errorMessage.value=null;currentPage.value=1
}

/* table actions */
function removeRow(i:number){parsedData.value.splice(i,1); if(currentPage.value>totalPages.value) currentPage.value=totalPages.value}
...
function confirmData(){
  ...
  errorMessage.value=null; showConfirm.value=true
}

async function sendToBackend(){
  isSubmitting.value=true
  try{
    await api.post('/students/bulk',parsedData.value,{params:{schoolYear:schoolYear.value}})
    toast.success(`Liste ${schoolYearLabel.value} enregistrée`) ; showConfirm.value=false; router.push({name:'Dashboard'})
  }catch(e:any){toast.error(e.response?.data?.message||'Erreur serveur')}finally{isSubmitting.value=false}
}

/* modal add */
function openAddChildModal(){modalErrorMessage.value=''; newChildNom.value=''; newChildPrenom.value=''; showModal.value=true}
function closeAddChildModal(){showModal.value=false}
function confirmAddChild(){if(!isValidName(newChildNom.value)||!isValidName(newChildPrenom.value)){modalErrorMessage.value='Nom/prénom non valides';return} parsedData.value.push({nom:newChildNom.value.trim(),prenom:newChildPrenom.value.trim()}); closeAddChildModal()}
</script>
