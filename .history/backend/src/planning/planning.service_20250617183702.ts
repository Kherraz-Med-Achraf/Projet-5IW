...
import {
  ...
  export class PlanningService {
    ...
    async submitPlanning(semesterId: string): Promise<void> {
      ...
      // ici on pourrait, par exemple, passer un flag "published" sur Semester
    }
    ...
    async getImportedExcelBuffer(
      ...
      // on récupère les entrées stockées
      const entries = await this.prisma.scheduleEntry.findMany({
        where: { semesterId, staffId },
        include: { entryChildren: { include: { child: true } } },
      ...
      // on construit un tableau de lignes
      const aoa: any[][] = [];
      // en-tête
      aoa.push(['dayOfWeek', 'startTime', 'endTime', 'activity', 'children']);
      // lignes
      for (const e of entries) {
        const childrenNames = e.entryChildren
    ...
    /** 9. Prévisualisation : parse, validate et retourne DTO */
    async previewExcel(
      ...
      // on effectue un Promise.all pour permettre l’await dans la boucle
      return Promise.all(
        rows.map(async r => {
          const start = await this.parseTime(
            semesterId,
            r.dayOfWeek,
            r.startStr,
          );
          const end = await this.parseTime(
            semesterId,
            r.dayOfWeek,
            r.endStr,
          );
    ...
    /** 10. Import définitif : rollback en cas d’erreur, puis persist */
    async importExcel(
      file: Express.Multer.File,
      semesterId: string,
      staffId: string,
