...
export class ChatService {
  ...
  async findAllForUser(userId: string) {
    ...
    for (const result of results) {
      ...
      if (uniqueChats.has(participantsKey)) {
        ...
        if (new Date(result.updatedAt) > new Date(existing.updatedAt)) {
  ...
  async getAllowedContacts(userId: string, role: string): Promise<Contact[]> {
        ...
        const adminUsers = await this.prisma.user
          .findMany({
            ...
            include: {
              ...
              // Supprimer parentProfile car DIRECTOR ne peut pas parler aux PARENT
            },
          });
    ...
    // Déduplication renforcée avec vérification du nom et logging
    ...
    for (const contact of contacts) {
      ...
      if (contactsMap.has(key)) {
        duplicates.push(`${contact.name} (${contact.id})`);
      } else {
        contactsMap.set(key, contact);
      }
    }
    
    // Log des doublons détectés si il y en a
    if (duplicates.length > 0) {
      this.logger.warn(`Doublons détectés dans les contacts pour user ${userId} (role: ${role}): ${duplicates.join(', ')}`);
    }
    ...
    this.logger.debug(`Contacts pour user ${userId} (role: ${role}): ${uniqueContacts.length} contacts après déduplication`);
