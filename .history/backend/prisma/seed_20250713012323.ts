...
async function main() {
  ...
  /* ---------------------------------------------------------------------- */
  /* 6.1. PARENT SPÉCIAL AVEC MOT DE PASSE EXPIRÉ (PARENT20)               */
  /* ---------------------------------------------------------------------- */
  
  // Créer parent20 avec mot de passe expiré pour tester le changement forcé
  const expiredDate = new Date();
  expiredDate.setDate(expiredDate.getDate() - 65); // 65 jours en arrière (> 60 jours)
  
  const parent20User = await prisma.user.create({
    data: {
      email: 'parent26@example.com',
      password: await hash(DEFAULT_PWD),
      role: 'PARENT',
      emailVerified: true,
      passwordChangedAt: expiredDate, // Mot de passe expiré
      forcePasswordReset: false, // Ne pas forcer manuellement, laisser l'expiration automatique
    },
  });
  
  const parent20Profile = await prisma.parentProfile.create({
    data: {
      userId: parent20User.id,
      firstName: 'Test',
      lastName: 'MotDePasseExpiré',
      phone: '0620000000',
      address: '20 rue du Test Expiration, 94000 Créteil',
      legalResponsibility: 'Père/Mère',
    },
  });

  // Créer un enfant pour parent20
  const parent20Child = await prisma.child.create({
    data: {
      firstName: 'Enfant',
      lastName: 'TestExpiration',
      birthDate: new Date(2015, 5, 20),
      parentProfileId: parent20Profile.id,
    },
  });
  allChildren.push(parent20Child);

  // Créer le contact d'urgence pour parent20
  await prisma.emergencyContact.create({
    data: {
      parentProfileId: parent20Profile.id,
      name: 'Contact Urgence Test',
      phone: '0720000000',
      relation: 'Grand-parent',
    },
  });

  console.log('✅ Parent26 créé avec mot de passe EXPIRÉ → parent26@example.com / Test1234!!!! (expiré depuis 65 jours)');
  ...
  /* ---------------------------------------------------------------------- */
  /* 7. COURS INTERACTIFS - SUPPRIMÉ                                        */
  /* ---------------------------------------------------------------------- */
  // Section supprimée car les cours sont maintenant gérés côté frontend
