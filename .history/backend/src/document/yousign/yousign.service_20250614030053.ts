...
export class YousignService {
  ...
  /* -------------------------------------------------------------- *
   * 1. Création de la signature-request (toujours « email »)
   * -------------------------------------------------------------- */
  ...
  /* -------------------------------------------------------------- *
   * 2. Upload du PDF (multipart) – on force une vraie ext .pdf
   * -------------------------------------------------------------- */
  ...
  /* -------------------------------------------------------------- *
   * 3. Ajout d’un signer – signature électronique + validation e-mail
   * -------------------------------------------------------------- */
  private addSigner(
    reqId: string,
    ...
    return this.call<SignerRes>(
      `signature_requests/${reqId}/signers`,
      {
        ...
        json  : {
          ...
          signature_level   : 'electronic_signature', // niveau « simple »
          authentication_mode: 'email',               // validation par mail
          redirect_url       : this.FRONT,            // retour appli
        },
      },
    );
  }
  ...
  /* ============================================================= *
   * Méthodes utilisées par DocumentService
   * ============================================================= */

  /**
   * Lance la procédure Yousign :
   *  - consent  → entité BD (pour le nom)
   *  - filePath → PDF temporaire
   *  - secretaryEmail / parentEmail → destinataires réels
   */
  async createProcedure(
    ...
    /* 3. Signers (l’ordre a de l’importance) */
  ...
  /**
   * Récupère l’URL embarquée du secrétaire (index 0) ou du parent (index 1)
   */
