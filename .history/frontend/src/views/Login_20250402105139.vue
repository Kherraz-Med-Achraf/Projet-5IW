...
export default {
  ...
  data() {
    return {
      email: '',
      password: '',
      // Déclare auth dans data pour le stocker
      auth: null
    }
  },
  created() {
    // Initialise le store auth au moment de la création du composant
  ...
  methods: {
    async onSubmit() {
      ...
      if (this.auth.token) {
        ...
        this.$toast.error(this.auth.error || 'La connexion a échoué.')
      }
    }
  }
}
