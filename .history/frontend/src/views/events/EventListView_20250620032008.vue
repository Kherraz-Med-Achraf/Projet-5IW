  ...
  <div class="event-list">
    ...
    <div v-for="ev in eventStore.events" :key="ev.id" class="card">
      ...
      <div class="content">
        ...
        <p v-if="ev.capacityLeft !== null"><strong>Places restantes :</strong> {{ ev.capacityLeft }}</p>
        <p v-if="registeredChildren(ev).length" class="already">
          Déjà inscrit: {{ registeredChildren(ev).join(', ') }}
        </p>
        <button @click="openModal(ev)" :disabled="registeredChildren(ev).length || ev.capacityLeft===0">S'inscrire</button>
      </div>
    </div>
