  ...
  export class PlanningService {
    ...
    async importExcel(
      ...
      const holidays = [
        ...
        new Date(y + 1, 0, 1), new Date(y + 1, 4, 1), new Date(y + 1, 4, 8),
        new Date(y + 1, 6, 14), new Date(y + 1, 7, 15), new Date(y + 1, 10, 1),
        new Date(y + 1, 10, 11), new Date(y + 1, 11, 25),
      ].map(d => d.toDateString());
      const isHoliday = (d: Date) =>
        holidays.includes(d.toDateString());
  
      // 5) date du premier lundi-type
      const mondayTpl = template.find(
        e => e.dayOfWeek === 1 && e.startTime.endsWith('09:00:00.000Z'),
      );
      if (!mondayTpl) {
        throw new BadRequestException('Impossible de déterminer le lundi type');
      }
      const firstMonday = new Date(mondayTpl.startTime);
  
      // 6) itération hebdomadaire
      const msDay  = 24 * 60 * 60 * 1000;
      const msWeek = 7 * msDay;
      const nbWeeks = Math.ceil((semEnd.getTime() - firstMonday.getTime()) / msWeek) + 1;
      ...
      await this.prisma.$transaction(async tx => {
        ...
        for (let w = 0; w < nbWeeks; w++) {
          for (const tpl of template) {
            const s0 = new Date(tpl.startTime).getTime();
            const e0 = new Date(tpl.endTime).getTime();
            const start = new Date(s0 + w * msWeek);
            const end   = new Date(e0 + w * msWeek);
