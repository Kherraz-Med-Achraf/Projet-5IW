...
export class ServiceManagerService {
  ...
  async create(dto: CreateServiceManagerDto) {
    ...
    const duplicate = await this.prisma.serviceManagerProfile.findFirst({
      where: { firstName, lastName },
    });
    if (duplicate) {
      throw new BadRequestException(
        `Un chef de service "${firstName} ${lastName}" existe déjà.`,
      );
    }
  ...
  async update(id: number, dto: UpdateServiceManagerDto) {
    ...
    if (
      ...
      if (duplicate) {
        throw new BadRequestException(
          `Un autre chef de service "${newFirst} ${newLast}" existe déjà.`,
        );
      }
    }
