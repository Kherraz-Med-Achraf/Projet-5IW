...
const loadHomeData = async () => {
  ...
  // Petit délai pour s'assurer que l'auth store est stabilisé
...
onMounted(async () => {
  ...
  // Charger les données seulement si l'utilisateur est déjà prêt
  if (auth.user) {
    await loadHomeData();
  }
});

// Watcher sur l'utilisateur pour charger les données quand il est prêt
watch(
  () => auth.user,
  (newUser) => {
    if (newUser && route.name === 'Home') {
      console.log('Home: User ready, loading home data...');
      loadHomeData();
    }
  },
  { immediate: true }
...
// Watcher sur la route pour recharger les données
watch(
  () => route.name,
  (newRouteName) => {
    if (newRouteName === 'Home' && auth.user) {
      console.log('Home: Route changed to Home, reloading data...');
      loadHomeData();
    }
  },
  { immediate: true }
