<template>
  <div class="p-4 max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold mb-4">Appels des présences – {{ todayLabel }}</h1>

    <!-- Loader -->
    <div v-if="loading" class="text-center py-8">Chargement…</div>

    <!-- Message si déjà validé -->
    <div v-else-if="sheet?.validatedAtStaff" class="mb-6 p-4 bg-green-100 rounded">
      <p>
    ...
    <!-- Table des enfants + Checkbox (phase STAFF) -->
    <table
      v-else
...
const staffName = computed(() => {
  const s = sheet.value?.staff?.staffProfile;
