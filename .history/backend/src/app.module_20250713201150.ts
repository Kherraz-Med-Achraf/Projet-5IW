...
/**
 * Fonction pour lire un secret depuis un fichier Docker ou depuis une variable d'environnement
 */
function readSecret(filePath: string, envVarName: string): string {
  ...
  // Sinon, lire depuis le fichier secret
  try {
    return fs.readFileSync(filePath, 'utf8').trim();
  } catch (err) {
    throw new Error(`Impossible de lire le secret depuis ${filePath} ou ${envVarName}`);
  }
}
...
@Module({
  imports: [
    ...
    // Global rate-limiting : 30 requêtes / minute par IP
