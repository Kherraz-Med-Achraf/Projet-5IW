    ...
    for (const child of allChildren) {
      ...
      for (const { year, month, academicYear: academicYearLabel } of periods) {
        ...
        if (!existingJournal) {
          ...
          // Déterminer le statut du journal (plus réaliste selon l'ancienneté)
          const monthDate = new Date(year, month - 1, 15); // milieu du mois
          const monthsAgo = Math.max(0, Math.floor((currentDate.getTime() - monthDate.getTime()) / (1000 * 60 * 60 * 24 * 30)));
          
          // Plus le mois est ancien, plus il a de chances d'être soumis
