  ...
  <main class="profile-container" role="main" lang="fr">
    <div class="profile-content">
      ...
      <div v-else class="content-grid">
        <!-- PageHeader -->
        <PageHeader 
          :title="`Journal mensuel - ${monthLabel}`" 
          :subtitle="`${childName} - ${yearLabel}`"
          icon="edit_note"
          :statusText="isSubmitted ? 'Soumis' : isDraft ? 'Brouillon' : 'Nouveau'"
          :statusIcon="isSubmitted ? 'check_circle' : isDraft ? 'edit' : 'add_circle'"
          :statusClass="isSubmitted ? 'status-validated' : isDraft ? 'status-pending' : 'status-info'"
        />

        <!-- Action button for non-submitted drafts -->
        <div v-if="!isSubmitted && isDraft" class="action-section">
          <button @click="submitJournal" :disabled="submitting" class="edit-btn edit-btn-success">
        ...
        <section class="profile-section">
          ...
          <div class="section-content">
            <div class="info-note" v-if="!isSubmitted">
              ...
              <div v-for="mission in missions" :key="mission.id" class="mission-card">
                <div class="mission-content">
                  ...
                  <div class="mission-info">
                    ...
                    <textarea
                      v-model="form.progressionMissions[mission.id.toString()]"
                      :disabled="isSubmitted"
                      rows="3"
            ...
            <div v-for="att in attachments" :key="att.id" class="attachment-card">
              <div class="attachment-header">
                ...
                <div class="attachment-info">
                  <h4>{{ getAttachmentName(att) }}</h4>
                  <span class="attachment-type">{{ getAttachmentType(att) }}</span>
                </div>
                <div class="attachment-actions">
                  <button @click="viewAttachment(att)" class="edit-btn edit-btn-sm">
                    <span class="material-icons">visibility</span>
                    Voir
                  </button>
...
const form = reactive({
  contenu: "",
  progressionMissions: {} as Record<string, string>,
});
...
onMounted(async () => {
  try {
    ...
    if (existingJournal.value) {
      ...
      // Correction pour la progression des missions
      const progressionData = existingJournal.value.progressionMissions || {};
      form.progressionMissions = {};
      
      // Si c'est un objet avec des clés numériques, on le transforme correctement
      if (typeof progressionData === 'object' && progressionData !== null) {
        Object.keys(progressionData).forEach(key => {
          const value = progressionData[key];
          // S'assurer que la valeur est une chaîne de caractères
          if (typeof value === 'string') {
            form.progressionMissions[key] = value;
          } else if (typeof value === 'object' && value !== null) {
            // Si c'est un objet, on prend la propriété 'contenu' ou 'text' ou on stringify
            form.progressionMissions[key] = value.contenu || value.text || value.value || JSON.stringify(value);
          } else {
            form.progressionMissions[key] = String(value || '');
          }
        });
      }
...
async function onProposeProgress(m: any) {
  ...
  const current = form.progressionMissions[m.id.toString()] || "";
  if (!current.trim()) {
...
function acceptMissionProposal(m: any) {
  if (m.aiProposal) {
    form.progressionMissions[m.id.toString()] = m.aiProposal;
    m.aiPropose = false;
    m.aiProposal = null;
  }
}
...
async function saveJournal() {
  ...
  saving.value = true;
  try {
    // Nettoyage des données de progression des missions
    const cleanProgressionMissions: Record<string, string> = {};
    Object.keys(form.progressionMissions).forEach(key => {
      const value = form.progressionMissions[key];
      if (value && typeof value === 'string' && value.trim() !== '') {
        cleanProgressionMissions[key] = value.trim();
      }
    });
...
function getAttachmentName(att: any): string {
  // Basé sur l'interface Journal du store : { filename: string, filepath: string }
  return att.filename || att.originalName || att.fileName || att.name || 'Fichier sans nom';
}

function getAttachmentType(att: any): string {
  // Extraire le type depuis l'extension du fichier
  const filename = att.filename || att.originalName || att.fileName || att.name || '';
  ...
  return typeMap[extension || ''] || att.type || att.mimeType || att.mimetype || att.fileType || 'Type inconnu';
}

async function viewAttachment(att: any) {
  console.log('Viewing attachment:', att);
  
  // Construire l'URL basée sur la structure du backend
  let url = '';
  
  // Le backend utilise une route sécurisée pour servir les fichiers
  // Route: GET /journal/file/:filename
  if (att.filename) {
    // Utiliser la route sécurisée du backend
    url = `${API_BASE_URL}/journal/file/${att.filename}`;
  } else if (att.filepath) {
    // Si on a un filepath, extraire le nom du fichier
    const filename = att.filepath.split('/').pop() || att.filepath;
    url = `${API_BASE_URL}/journal/file/${filename}`;
  } else if (att.id) {
    // Fallback : utiliser l'API d'attachments par ID
    url = `${API_BASE_URL}/journal/attachment/${att.id}`;
  }
  ...
  if (url) {
    try {
      // Utiliser l'API sécurisée pour récupérer le fichier
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          toast.error(`Erreur lors de l'ouverture du fichier: ${response.statusText}`);
        }
        return;
      }
      ...
      // Nettoyer l'URL du blob après un délai (pour éviter les fuites mémoire)
      setTimeout(() => {
        URL.revokeObjectURL(blobUrl);
      }, 5000);

      if (!newWindow) {
        toast.warning('Veuillez autoriser les pop-ups pour ouvrir le fichier');
      }
    ...
    console.error('Attachment data:', att);
    toast.error('Impossible d\'ouvrir le fichier : données d\'attachment incomplètes');
  }
}
...
/* Contenu des sections avec padding */
.section-content {
  padding: 2rem;
}
