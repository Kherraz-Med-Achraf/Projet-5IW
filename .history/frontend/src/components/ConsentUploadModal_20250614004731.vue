...
<template>
  ...
  interface ParentFull { id: string; firstName: string; lastName: string }
  ...
  /* header JWT si besoin (pas indispensable avec credentials: include) */
  const auth    = useAuthStore()
  const token   = auth.token
  const headers = token ? { Authorization: `Bearer ${token}` } : {}
  
  /* charge la liste complète des parents */
  onMounted(async () => {
    try {
      ...
      if (res.ok) {
        parents.value = (await res.json()) as ParentFull[]
      } else {
