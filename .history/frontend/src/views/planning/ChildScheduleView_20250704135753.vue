...
const calendarEvents = computed(() => {
  ...
  // Marquer seulement le premier événement de chaque groupe comme visible
  Object.values(duplicateGroups).forEach(group => {
    group.sort((a, b) => a.start.localeCompare(b.start))
    group.forEach((ev, idx) => {
      ev.extendedProps.showEvent = idx === 0
    })
  })
...
const calendarOptions = computed(() => ({
  ...
  eventContent(info: any) {
    // Masquer les événements dupliqués dans toutes les vues
    if (!info.event.extendedProps.showEvent) {
      return { domNodes: [] }
    }
