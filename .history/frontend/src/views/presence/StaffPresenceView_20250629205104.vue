...
function initGrid() {
  ...
  const data = sheet.value.records.map(rec => [
    rec.child.lastName,
    rec.child.firstName,
    rec.childId, // Hidden for checkbox logic
    rec.present,
  ...
  const columns = [
    { name: 'Nom', width: '25%' },
    { name: 'Prénom', width: '25%' },
    { name: 'childId', hidden: true },
    {
      name: 'Présent',
      width: '15%',
      formatter: (cell: any, row: any) => {
        const childId = row.cells[2].data
        const isPresent = cell
        const justificationType = row.cells[4].data
        ...
        if (isValidated) {
          ...
          const checked = store.presentChildIds.includes(childId) ? 'checked' : ''
          return html(`
            <div class="checkbox-container">
              ...
              />
              <label class="checkbox-label">${store.presentChildIds.includes(childId) ? 'Présent' : 'Absent'}</label>
            </div>
          `)
        }
      }
...
function setupGlobalCheckboxHandler() {
  ;(window as any).handlePresenceChange = (childId: string, checked: boolean) => {
    // Convert childId to string to ensure consistency
    const childIdStr = String(childId)
    
    if (checked) {
      if (!store.presentChildIds.includes(childIdStr)) {
        store.presentChildIds.push(childIdStr)
      }
    } else {
      const index = store.presentChildIds.indexOf(childIdStr)
      if (index > -1) {
