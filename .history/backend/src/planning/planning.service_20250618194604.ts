...
import {
  ...
  export class PlanningService {
    ...
    /**
     * 9. Prévisualisation + validations globales, avec prise en charge de "pause"
     */
    async previewExcel(
      ...
      for (const sheetName of sheetNames) {
        ...
        // Itérer chaque ligne (à partir de 2ᵉ)
        for (let i = 1; i < rows.length; i++) {
          ...
          for (let idx = 1; idx <= lastIdx; idx++) {
            const cellRaw = (row[idx] || '').toString().trim();
            ...
            // Séparer "Activité – Enfants"
            const parts = cellRaw.split('–').map(s => s.trim());
            ...
            // Extraire & filtrer les noms (supprimer les vides)
            ...
            // Validation existence des enfants
            ...
            // Marquer couverture matin/après‐midi
      ...
      // 2) tous les STAFF en base doivent apparaître
      ...
      // 3) vérification coverage enfants
      const errors: string[] = [];
      for (let dow = 1; dow <= 5; dow++) {
    ...
    async importExcel(
      ...
      await fs.writeFile(
        path.join(this.uploadDir, `${semesterId}.xlsx`),
        file.buffer,
      );
