...
const formattedError = computed(() => {
  ...
  try {
    ...
    if (errorObj.details && Array.isArray(errorObj.details)) {
      return `
        <p>Les enfants suivants n'ont pas de créneaux complets :</p>
        <ul style="margin-top: 0.5rem; padding-left: 1.5rem;">
          ${errorObj.details.map((detail: string) => `<li>${detail}</li>`).join('')}
        </ul>
        <p style="margin-top: 0.5rem; font-size: 0.9em; color: #6b7280;">
          Veuillez vérifier votre fichier Excel et vous assurer que tous les enfants ont des créneaux pour chaque jour de la semaine.
        </p>
      `
    }
  } catch (e) {
    // Si ce n'est pas du JSON, afficher l'erreur telle quelle
  }
