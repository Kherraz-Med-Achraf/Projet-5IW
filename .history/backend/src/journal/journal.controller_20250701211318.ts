    ...
    @UseInterceptors(
      FileInterceptor('file', {
        ...
        fileFilter: (_req, file, cb) => {
          if (!file.mimetype.match(/\/(pdf|jpeg|png|jpg|doc|docx)$/)) {
            return cb(new BadRequestException('Type de fichier non autorisé. Formats acceptés : PDF, DOC, DOCX, JPG, PNG'), false);
          }
          cb(null, true);
        },
