...
// Computed properties
const recentChats = computed(() => chatStore.chats);
...
function displayName(chat: any): string {
  ...
  // Filtrer les participants pour exclure l'utilisateur actuel
  const otherParticipants = chat.participants.filter((p: any) => {
    const participantId = p.id || p._id || p;
    return participantId !== userId;
  });
  
  // Mapper les noms des participants
  const names = otherParticipants.map((p: any) => {
    // Si le participant a déjà un nom
    if (p.name) return p.name;
    
    // Sinon, chercher dans les contacts
    const participantId = p.id || p._id || p;
    const contact = contacts.value.find(c => c.id === participantId);
    
    if (contact) return contact.name;
    
    // Si pas trouvé, utiliser l'ID comme fallback
    return participantId || "Utilisateur";
  });
...
async function send() {
  ...
  chatStore.send(activeId.value, content);
  scrollToBottom();
}
...
async function startChat(contactId: string) {
  try {
    // Créer une version personnalisée sans redirection
    const myId = auth.user?.id || "";
    
    // Vérifier si une conversation existe déjà
    const existing = recentChats.value.find(
      (c) => c.participants.some((p: any) => p.id === myId) && 
             c.participants.some((p: any) => p.id === contactId)
    );
    ...
    if (existing) {
      ...
      showNew.value = false;
      await chatStore.fetchMessages(existing.id);
      chatStore.join(existing.id);
      scrollToBottom();
      return;
    }
    ...
    const res = await secureJsonCall(`${API}/chats`, {
      method: "POST",
      body: JSON.stringify({ participants: [myId, contactId] }),
    });
    
    const chatId = res.id || res._id;
    if (!chatId) throw new Error("ID manquant dans la réponse");
