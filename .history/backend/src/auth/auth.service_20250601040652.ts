...
export class AuthService {
  ...
  async registerParent(dto: RegisterParentDto) {
    ...
    /* 1) user + profil parent (sans enfants) */
    const parent = await this.prisma.user.create({
      data: {
        email,
        password: hash,
        role:       Role.PARENT,
        emailVerified: true,           // plus de vérification d’e-mail
        parentProfile: {
          create: {
            ...parentData,
      ...
      include: { parentProfile: true }, // ← on veut l’id du profil parent
    });
  
    /* 2) création de chaque enfant + compte associé */
    for (const ch of children) {
      await this.childService.createForParent(
        parent.parentProfile.id,
        {
          firstName : ch.firstName,
          lastName  : ch.lastName,
          birthDate : ch.birthDate, // string ISO (CreateChildDto)
        },
      );
    }
  
    return { message: 'Compte parent (et enfants) créé et actif immédiatement.' };
  }
