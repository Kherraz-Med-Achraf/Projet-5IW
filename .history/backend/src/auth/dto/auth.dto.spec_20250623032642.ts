...
describe('Auth DTOs', () => {
  ...
  describe('ResetPasswordDto', () => {
    ...
    it('should fail validation when prid is missing', async () => {
      const resetPasswordData = {
        token: 'reset-token-456',
        newPassword: 'NewPassword123!',
      };
      ...
      expect(errors).toHaveLength(1);
      expect(errors[0].property).toBe('prid');
      expect(errors[0].constraints).toHaveProperty('isNotEmpty');
    });
    ...
    it('should fail validation when newPassword is missing', async () => {
      ...
      const resetPasswordDto = plainToClass(ResetPasswordDto, resetPasswordData);
      const errors = await validate(resetPasswordData);
    ...
    it('should handle whitespace trimming', async () => {
      ...
      expect(errors).toHaveLength(0);
      expect(resetPasswordDto.prid).toBe('reset-id-123');
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle maximum length constraints', async () => {
      ...
      // Should pass validation (no max length constraint defined)
      expect(errors).toHaveLength(0);
    });
  });
}); 