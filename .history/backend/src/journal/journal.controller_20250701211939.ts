...
import {
  ...
  export class JournalController {
    ...
    private generateSafeFilename(originalname: string): { diskFilename: string; displayName: string } {
      // Fonction de translittération plus robuste
      ...
      // Nettoie le nom d'affichage
      let displayName = transliterate(originalname)
      ...
      if (!displayName || displayName.length < 1) {
        displayName = `document_${timestamp}${extension}`;
      }
