...
function confirmData() {
  ...
  if (invalid.length) {
    errorMessage.value = 'Élèves invalides : ' + invalid.map(r => `${r.nom} ${r.prenom}`).join(', ')
    return
  }
  errorMessage.value = null
...
async function sendToBackend() {
  ...
  try {
    const res = await fetch(`http://localhost:3000/students/annual-import?schoolYear=${schoolYear.value}`, {
      method: 'POST',
