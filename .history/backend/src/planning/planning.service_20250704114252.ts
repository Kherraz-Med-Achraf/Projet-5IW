...
export class PlanningService {
  ...
  async previewExcel(
    ...
    for (const sheetName of sheetNames) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (!staffId) {
          // Essayons plusieurs variantes pour plus de debug
          const variations = [
            key,
            key.split(' ').reverse().join(' '), // Inverse prénom/nom
            key.split(' ')[0], // Premier mot seulement
            key.split(' ').slice(-1)[0], // Dernier mot seulement
          ];
  ...
  async importExcel(
    ...
    // Stocker le nom du fichier en base pour le retrouver plus tard
    await this.prisma.semester.update({
      where: { id: semesterId },
      data: { 
        planningFilename: secureFilename 
      }
    });
  }
