...
<template>
   ...
   * Suppression du style “grisé” pour les mois passés ou en cours :
   * on les affiche tous de la même façon (editable) sauf si déjà soumis.
   */
  function monthClass(month: number) {
    if (!selectedYearId.value || !selectedChildId.value) {
      // Tant que l'année ou l'enfant n'est pas sélectionné, on désactive
    ...
    if (found?.isSubmitted) {
      // Mois déjà soumis → fond gris clair
      return 'cursor-pointer bg-gray-100'
    } else {
      // • Mois non soumis (même s'il est passé ou futur) → fond bleu clair, cliquable
  ...
  function isMonthEditable(month: number) {
    ...
    // Si avenir → non éditable ; si passé ou en cours, on l'affiche comme editable
