<template>
  <div class="max-w-3xl mx-auto p-6 space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <!-- Un seul attribut type="email" ici, pas deux -->
      <BaseInput
        v-model="form.email"
        label="Email"
        type="email"
...
onMounted(async () => {
  ...
  try {
    ...
    toast.error(e.message || 'Lien invalide ou expiré')
    router.push({ name: 'Login' })
  } finally {
...
async function submit(){
  ...
  try {
    const response = await fetch('http://localhost:3000/auth/register-by-invite', {
      method: 'POST',
