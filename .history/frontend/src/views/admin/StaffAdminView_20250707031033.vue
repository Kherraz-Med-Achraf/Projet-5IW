        ...
        <section class="profile-section" id="staff-table" aria-labelledby="staff-title">
          ...
          <div class="section-content">
            ...
            <div class="grid-container">
              <div ref="gridWrapper" v-show="staff.length"></div>
...
async function fetchStaff() {
  ...
  try {
    ...
    staff.value = [
      { id: 1, firstName: 'Alice', lastName: 'Dubois', email: 'alice.dubois@exemple.com', createdAt: '2024-01-15' },
      { id: 2, firstName: 'Bob', lastName: 'Moreau', email: 'bob.moreau@exemple.com', createdAt: '2024-02-20' },
    ];
    if (grid.value) {
      grid.value.updateConfig({ data: staff.value }).forceRender();
    }
  } catch (err: any) {
...
function initGrid() {
  grid.value = new Grid({
        ...
        formatter: (cell, row) => {
          ...
          return html(`
            <div class="action-buttons">
              <button class="btn-icon edit-btn" data-id="${staffMember.id}" title="Modifier">
                <i class="material-icons">edit</i>
              </button>
              <button class="btn-icon delete-btn" data-id="${staffMember.id}" title="Supprimer">
    ...
    pagination: { limit: 10 },
    language: {
      search: { placeholder: 'Rechercher un membre du personnel...' },
      pagination: {
        previous: 'Précédent',
        next: 'Suivant',
