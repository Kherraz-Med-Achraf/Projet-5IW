...
<template>
  <div class="planning-upload">
    ...
    <!-- Pour debug, affichage des données -->
    <pre v-if="debug">{{ JSON.stringify({ previewEntries, staffList, filteredEvents }, null, 2) }}</pre>
...
// expose a debug flag to toggle the <pre> block
...
// log data each time ils changent
watch([previewEntries, staffList, filteredEvents, selectedStaffId], () => {
  console.log('previewEntries:', previewEntries.value)
  console.log('staffList:', staffList.value)
  console.log('selectedStaffId:', selectedStaffId.value)
...
// on mount, load semesters and staff
onMounted(async () => {
