  ...
  actions: {
    ...
    async validateSheet() {
      ...
      try {
        ...
        console.log('📡 Store: Envoi de la requête', {
          url,
          payload,
        ...
        console.log('📨 Store: Réponse reçue', {
          status: res.status,
        ...
        if (!res.ok) {
          ...
          console.error('❌ Store: Erreur API', { status: res.status, errorText });
          throw new Error(errorText || 'Erreur lors de la validation');
        }
        
        const newSheet = await res.json() as PresenceSheet;
