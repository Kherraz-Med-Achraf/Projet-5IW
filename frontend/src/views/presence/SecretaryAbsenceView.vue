...
async function openFile(path: string) {
  if (!path) {
    notify.showNotification('Aucun fichier disponible', 'error')
    return
  }
  ...
  try {
    ...
    notify.showNotification('Erreur lors de l\'ouverture du fichier', 'error')
  }
}
...
async function downloadFile(path: string, suggestedName: string) {
  ...
  try {
    ...
    notify.showNotification('Erreur lors du téléchargement', 'error')
  }
}
...
async function submitJustification(payload: any) {
  try {
    await store.justifyRecord(payload.recordId, payload.type, payload.justificationDate, payload.motif, payload.file)
    notify.showNotification('Justification soumise avec succès', 'success')
    closeModal()
    store.fetchSheet()
  } catch (error) {
    notify.showNotification('Erreur lors de la soumission de la justification', 'error')
  }
}
