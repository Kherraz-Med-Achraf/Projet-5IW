...
export class PresenceService {
  ...
  async createSheet(dateString: string, staffId: string) {
    ...
    // 3) Retourne la feuille complète avec les enregistrements et l’utilisateur qui l’a créée
    return this.prisma.presenceSheet.findUnique({
      where: { id: sheet.id },
      include: {
        // inclut tous les records et, pour chaque record, l’enfant et sa justification
        records: { include: { child: true, justification: true } },
        // inclut le profil du staff ayant créé ou validé la feuille
        staff: {
          select: {
            staffProfile: {
  ...
  async validateSheet(
    ...
    // Met à jour tous les enregistrements en les recréant
    const children = await this.prisma.child.findMany();
