...
describe.skip('PresenceService', () => {
  ...
  it('createSheet génère la feuille et les records manquants', async () => {
    ...
    expect((sheet as any).records).toHaveLength(2);
  });
  ...
  it('justify crée une justification et valide la feuille quand tout est justifié', async () => {
    ...
    const initial = await svc.createSheet(date, 's');

    // phase éducateur : feuille validée sans présents => tous absents
    await svc.validateSheet((initial as any).id, [], 's');
