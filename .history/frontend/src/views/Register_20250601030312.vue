<template>
  <div class="max-w-3xl mx-auto p-6 space-y-8">
    <!-- Loader pendant la validation du token -->
    <div v-if="loadingInvite" class="text-center py-10">
      <p>Chargement…</p>
    </div>

    <!-- Formulaire complet (3 étapes) -->
    <div v-else>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <!-- Email : pré-rempli et désactivé si token -->
        <BaseInput
          v-model="form.email"
          label="Email"
          type="email"
...
async function submit(){
  ...
  /* payload différent si invitation ou non */
  if(inviteToken.value){
    // mode invitation : on ne transmet QUE les infos de création de compte
    const payload = {
    ...
    // mode inscription classique : on valide le reste
