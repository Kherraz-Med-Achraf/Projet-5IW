  ...
  actions: {
    ...
    async fetchStaffList() {
      ...
      try {
        ...
        this.error = err.message || 'Erreur au chargement des membres du staff'
      } finally {
        this.loading = false
      }
    },
     ...
     * @param staffId id du staff choisi
     */
    async previewUpload(file: File, staffId: string) {
      ...
      try {
        ...
        const res = await fetch(
          `/planning/semesters/${this.selectedSemester.id}/upload/${staffId}`,
     ...
     * @param staffId id du staff (depuis le store utilisateur)
     */
