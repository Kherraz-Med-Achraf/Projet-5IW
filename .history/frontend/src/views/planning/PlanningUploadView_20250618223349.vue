...
/** 6) Quand on change de semestre, on centre la vue sur sa date de début **/
watch(selectedSemesterId, id => {
  const sem = planning.semesters.find(s => s.id === id)
  if (!sem) return

  // définit initialDate pour le premier montage
  calendarOptions.value.initialDate = sem.startDate

  // si FullCalendar est déjà monté, on force le gotoDate
  if (fcRef.value) {
...
/** 7) Dès qu’on choisit un staff, on injecte les events calculés **/
watch(
...
/** 8) Réinitialiser tout au changement de semestre **/
