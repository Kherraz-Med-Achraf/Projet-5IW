...
export class AuthService {
  ...
  async registerWithRole(
    ...
    // 2. Vérifier qu’il n’existe pas déjà un user avec ce mail
    if (await this.prisma.user.findUnique({ where: { email } })) {
    ...
    // 4. Si on doit créer un Parent (forcedRole = Role.PARENT)
      ...
      const user = await this.prisma.$transaction(async (tx) => {
        return tx.user.create({
          data: {
            ...
            parentProfile: {
              create: {
                ...parentData, // ex. firstName, lastName, phone, address, legalResponsibility
                emergencyContacts: {
    ...
    // 5. Sinon (STAFF, DIRECTOR, etc.), on crée juste le User avec le rôle
