...
const loadHomeData = async () => {
  ...
  try {
    ...
    if (auth.user?.role === 'DIRECTOR' || auth.user?.role === 'SERVICE_MANAGER') {
      ...
      await blogStore.fetchPosts();
      console.log('Home: Blog data loaded for DIRECTOR/SERVICE_MANAGER');
      
      // Charger les événements
      await eventStore.fetchEvents();
      console.log('Home: Events data loaded for DIRECTOR/SERVICE_MANAGER');
      
      // Charger les données de présence du jour
      await loadPresenceData();
      console.log('Home: Today presence data loaded for DIRECTOR/SERVICE_MANAGER');
    }
    ...
    if (auth.user?.role === 'SECRETARY') {
      // Charger les posts du blog
      await blogStore.fetchPosts();
      console.log('Home: Blog data loaded for SECRETARY');
      
      // Charger les données de présence du jour
      await loadPresenceData();
      console.log('Home: Today presence data loaded for SECRETARY');
    }
