...
export class AuthService {
  ...
  async registerWithRole(dto: RegisterByInviteDto, forcedRole: Role) {
    ...
    /* 4 : création d’un PARENT -------------------------------------------- */
    if (forcedRole === Role.PARENT) {
      /* a) Nettoyage des contacts d’urgence */
      const contactsToCreate = (emergencyContacts ?? [])
        .filter(
          (c) => c?.firstName && c?.lastName && c?.phone && c?.relation,
        )
        .map((c) => ({
      ...
      /* b) Nettoyage des enfants */
      const childrenToCreate = (children ?? [])
        .filter((ch) => ch?.firstName && ch?.lastName && ch?.birthDate)
        .map((ch) => ({
          firstName: ch.firstName.trim(),
          lastName: ch.lastName.trim(),
          birthDate: new Date(ch.birthDate),
        }));
  
      /* c) Transaction de création */
          ...
          parentProfile: {
            create: {
              ...
              ...(contactsToCreate.length && {
                emergencyContacts: { create: contactsToCreate },
              }),
              ...(childrenToCreate.length && {
                children: { create: childrenToCreate },
              }),
            },
          },
        },
