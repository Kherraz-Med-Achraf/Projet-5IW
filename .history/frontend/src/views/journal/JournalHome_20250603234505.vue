...
<template>
    <div class="p-4 max-w-4xl mx-auto">
      ...
      <div class="mb-6">
        <label for="child-select" class="block font-medium mb-1">
      ...
      <div v-if="selectedChildId" class="mb-6">
        <label for="year-select" class="block font-medium mb-1">
          Année scolaire :
        </label>
        <select
          id="year-select"
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          >
            <span class="block text-center py-2">
              {{ label }}
              <!-- ✔ pour mois soumis -->
              <span v-if="isMonthConsultable(index + 1)" class="text-green-600"
                >✔</span
              >
              <!-- 📋 pour mois ayant déjà un brouillon (non soumis) -->
              <span v-else-if="hasDraft(index + 1)" class="text-blue-600"
                >📋</span
              >
              <!-- ✎ pour tous les autres mois passés/en cours sans brouillon -->
              <span v-else-if="isMonthEditable(index + 1)" class="text-blue-600"
                >✎</span
              >
