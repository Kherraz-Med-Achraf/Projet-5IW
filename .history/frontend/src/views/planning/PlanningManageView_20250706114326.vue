...
async function handleCancelCourse(course: any) {
  ...
  if (course.cancelled) {
    try {
      ...
      alert('Cours réactivé avec succès ! Les enfants transférés ont été automatiquement restaurés.')
    } catch (error) {
      console.error('=== ERREUR RÉACTIVATION ===', error)
      // Afficher l'erreur à l'utilisateur
      alert('Erreur lors de la réactivation du cours: ' + (error.message || 'Erreur inconnue'))
    }
    return
  }
