...
import {
  ...
  export class PlanningService {
    private readonly uploadDir = path.join(process.cwd(), 'uploads', 'planning_staff');
    ...
    async getImportedExcelBuffer(
      ...
      const filePath = path.join(this.uploadDir, `${semesterId}_${staffId}.xlsx`);
      try {
    ...
    /** 10. Import définitif : rollback en cas d’erreur, puis persist + enregistrement du fichier */
    async importExcel(
      file: Express.Multer.File,
      semesterId: string,
      staffId: string,
    ): Promise<void> {
      // 10a. Écriture du fichier sur disque
      await fs.mkdir(this.uploadDir, { recursive: true });
      const filename = `${semesterId}_${staffId}.xlsx`;
      const fullPath = path.join(this.uploadDir, filename);
      await fs.writeFile(fullPath, file.buffer);
  
      // 10b. Validation puis transaction DB
      const rows = this.parseExcel(file.buffer);
      const allNames = Array.from(new Set(rows.flatMap(r => r.names)));
      const children = await this.prisma.child.findMany({
        where: {
