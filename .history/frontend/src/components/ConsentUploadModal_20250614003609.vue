...
<template>
  ...
  /* API call : on charge toute la liste une seule fois */
  onMounted(async () => {
    const res = await fetch('/parents') // renvoie tous les parents light (ou adapte)
    parents.value = (await res.json()) as ParentLight[]
  })
