...
export class DocumentController {
  ...
  async addDocumentAccess(
    ...
    // 🔧 FIX: S'assurer que les parentIds sont des numbers
    const parentIds = body.parentIds.map(id => typeof id === 'string' ? parseInt(id, 10) : Number(id)).filter(id => !isNaN(id))
    
    if (parentIds.length === 0) {
      throw new BadRequestException('IDs des parents invalides')
    }
    
    return this.documentService.addDocumentAccess(documentId, parentIds, req.user.id);
  }
  ...
  async removeDocumentAccess(
    ...
    return this.documentService.removeDocumentAccess(documentId, parentIds, req.user.id);
  }
