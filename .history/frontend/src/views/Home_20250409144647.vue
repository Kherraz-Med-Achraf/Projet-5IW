<template>
  <div>
    ...
    <!-- Affichage du QR code si OTP est activé -->
...
// Fonction pour générer le QR code à partir du secret OTP
...
// Lors du montage, on attend l'hydratation complète et on lit l'utilisateur stocké dans le localStorage
onMounted(async () => {
  await auth.hydrate()
  // On récupère l'utilisateur tel qu'il est stocké en localStorage
  const storedUser = localStorage.getItem('user')
  if (storedUser) {
    const parsedUser = JSON.parse(storedUser)
    // Si l'utilisateur a un otpSecret défini, on coche la case et on génère le QR code
...
// Surveille auth.user au cas où il changerait (optionnel)
watch(
  () => auth.user,
  async (newUser) => {
...
async function toggleOtp() {
  if (otpEnabled.value) {
    // Cas d'activation : si la case est cochée, on déclenche l'activation via API
    loading.value = true
    try {
      ...
      // On met à jour l'utilisateur dans le store
      if (auth.user) {
