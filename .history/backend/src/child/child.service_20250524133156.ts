...
async removeForParent(parentProfileId: number, id: number) {
    // 1) Supprimer le profil enfant (filter sur le parent)
    const deleteResult = await this.prisma.child.deleteMany({
      where: { id, parentProfileId },
    });
    ...
    // 2) Supprimer ensuite le compte utilisateur enfant, s’il existe
