...
// Compteurs - Les retards sont considérés comme des "présents"
const actualPresentCount = computed(() => presentRecords.value.length)
const latenessCount     = computed(() => justifiedLateness.value.length)
const presentCount      = computed(() => actualPresentCount.value + latenessCount.value) // Présents + Retards
...
async function openFile(path: string) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Erreur lors de l\'ouverture du fichier')
    }
