...
async function fetchSecretaries() {
  ...
  try {
    ...
    secretaries.value = [
      { id: 1, firstName: 'Emma', lastName: 'Rousseau', email: 'emma.rousseau@exemple.com', createdAt: '2024-01-15' },
      { id: 2, firstName: 'Lucas', lastName: 'Petit', email: 'lucas.petit@exemple.com', createdAt: '2024-02-20' },
    ];
    if (grid.value) {
      grid.value.updateConfig({ data: secretaries.value }).forceRender();
    }
  } catch (err: any) {
...
function initGrid() {
  grid.value = new Grid({
        ...
        formatter: (cell, row) => {
          ...
          return html(`
            <div class="action-buttons">
              <button class="btn-icon edit-btn" data-id="${secretary.id}" title="Modifier">
                <i class="material-icons">edit</i>
              </button>
              <button class="btn-icon delete-btn" data-id="${secretary.id}" title="Supprimer">
    ...
    data: secretaries.value,
    search: true,
    sort: true,
    pagination: { limit: 10 },
    language: {
      search: { placeholder: 'Rechercher un secrétaire...' },
      pagination: {
        previous: 'Précédent',
        next: 'Suivant',
