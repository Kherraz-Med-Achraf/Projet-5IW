      ...
      <div v-else class="missions-container">
        ...
        <section v-if="missionsList.length > 0" class="profile-section missions-section" aria-labelledby="missions-title">
          ...
          <header class="missions-header">
            <h1 id="missions-title">Missions annuelles</h1>
            <div class="child-info">
              <h2>{{ childName }}</h2>
              <p class="year-label">{{ yearLabel }}</p>
            </div>
            <div class="missions-stats" role="status" :aria-label="`${completedMissions} mission${completedMissions > 1 ? 's' : ''} renseignée${completedMissions > 1 ? 's' : ''} sur ${missionsList.length}`">
              <span class="completed-count">{{ completedMissions }}</span>
              <span class="total-count">/ {{ missionsList.length }}</span>
              <span class="stats-label">mission{{ missionsList.length > 1 ? 's' : '' }}</span>
            </div>
          </header>
          ...
          <div id="missions-list" class="missions-list" role="list" aria-label="Liste des missions annuelles">
            ...
            >
              ...
              <div class="mission-content">
                ...
                <div class="mission-input-container">
                  <label :for="`mission-textarea-${index}`" class="sr-only">
                  ...
                  ></textarea>
                  <div :id="`mission-${index}-help`" class="help-text">
                    Définissez un objectif SMART (Spécifique, Mesurable, Atteignable, Réaliste, Temporellement défini) pour cette mission annuelle
                  </div>
                  <button
            ...
            <div class="add-mission-section">
              <button
                ...
                type="button"
                :aria-describedby="missionsList.length >= 10 ? 'mission-limit-warning' : 'add-mission-help'"
                :disabled="missionsList.length >= 10"
              >
                <span class="material-icons" aria-hidden="true">add</span>
                Ajouter une mission
              </button>
              <div id="add-mission-help" class="help-text">
                Ajoutez jusqu'à 10 missions pour cette année scolaire
              </div>
        ...
        <section v-if="missionsList.length === 0" class="empty-missions" aria-labelledby="empty-title">
          <header class="mission-header-info">
            <h1 id="empty-title">Missions annuelles</h1>
            <div class="child-info">
              <h2>{{ childName }}</h2>
              <p class="year-label">{{ yearLabel }}</p>
            </div>
...
.missions-section {
  .missions-header {
    padding: 2.5rem;
    background: linear-gradient(135deg, #4444ac 0%, #3a3a9a 100%);
    color: white;
  ...
  .mission-item-unified {
    ...
    .mission-content {
      ...
      .mission-input-container {
        ...
        margin-bottom: 1rem;
        width: 100%;
        order: 1; // Ensure textarea appears first

        .mission-textarea {
          flex: 1;
          width: 100%;
          max-width: calc(100% - 60px); // Account for delete button + gap
      ...
      .ai-suggestion {
        ...
        margin-top: 1rem;
        transition: all 0.3s ease;
        width: calc(100% - 60px); // Same width as textarea (accounting for delete button + gap)
        box-sizing: border-box;
        order: 2; // Ensure it appears after textarea but before toggle
