...
watch(
  () => authStore.user,
  (newUser) => {
    if (newUser && route.name === 'Blog') {
      console.log('BlogView: User ready, loading blog data...');
      loadBlogData();
    }
  },
  { immediate: true }
...
watch(
  () => route.name,
  (newRouteName) => {
    if (newRouteName === 'Blog' && authStore.user) {
      console.log('BlogView: Route changed to Blog, reloading data...');
      loadBlogData();
    }
  },
  { immediate: true }
...
onMounted(() => {
  ...
  // Charger les posts seulement si l'utilisateur est déjà prêt
  if (authStore.user) {
    loadBlogData();
  }
});
